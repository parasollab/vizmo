...
public:
  ...
  shifted_first_pd(Op const& op, T nvalue)
    : m_op(op),
      m_neutral_value(nvalue)
  { }
  ...
  {
    if (id == 0) {
      visitor(m_op, constant_input(m_neutral_value),
              stapl::get<Indices+1>(in_flow).consume_from(make_tuple(id))...);
    }
    else {
      visitor(m_op, stapl::get<0>(in_flow).consume_from(make_tuple(id - 1)),
              stapl::get<Indices+1>(in_flow).consume_from(make_tuple(id))...);
    }
  }

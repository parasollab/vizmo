...
struct piped
{
  ...
  struct port_types
  {
    using in_port_size = typename While::body_p_type::in_port_size;
  ...
  in_flow(While const& while_p, In const&, std::size_t lid_offset,
          ...
          tags::while_continuation)
  {
    ...
    return while_p.template nested_out_port<in_flow_t,
      do_while_component::reduction>(lid_offset, while_p.current_iteration());
  }
  ...
  out_port(While const& while_p, std::size_t lid_offset)
  {
    ...
    return while_p.template nested_out_port<In, do_while_component::body>(
      lid_offset, while_p.dimensions() - 1);
  }
};

...
struct ph_list
{
  ...
  operator,(T) const
  {
    return ph_list<Phs...,T>();
  }
};


template<typename Skeleton, typename OutPh = use_default,
                            typename InPhs = use_default>
struct flow_tuple
{
  ...
  static constexpr bool needs_input_t=std::is_same<input_t,use_default>::value,
                      needs_output_t=std::is_same<output_t,use_default>::value;
...
public:
  ...
  template<typename... NewInPhs>
  typename std::enable_if<needs_input_t,
    flow_tuple<Skeleton, OutPh, ph_list<NewInPhs...>>>::type
  operator[](ph_list<NewInPhs...>)
  {
    return flow_tuple<Skeleton, OutPh, ph_list<NewInPhs...>>(get_skeleton());
  }

  template<typename NewInPh>
  typename std::enable_if<needs_input_t,
    flow_tuple<Skeleton, OutPh, ph_list<NewInPh>>>::type
  operator[](NewInPh)
  {
    return flow_tuple<Skeleton, OutPh, ph_list<NewInPh>>(get_skeleton());
  }
};
...
//////////////////////////////////////////////////////////////////////
/// @brief Replaces @c stapl::use_default with the appropriate value
/// based on the index of the skeleton.
///
/// If not specified, the skeleton reads from the x<I-1> or input<0> and writes
/// to x<0>.
///
/// @note The ternary operator is to avoid warnings when compiling with clang
/// in the case of I == 0.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
  template<size_t I>
  using fill_defaults_t =
    ph_flow<
      default_type<output_t, x<I>>,
      default_type<input_t,
        ph_list<typename std::conditional<I == 0, input<0>,x<I?I-1:I>>::type>>
    >;
...
struct ph_flows_tuple_of
{ };


template<typename... FlowTuples>
struct ph_flows_tuple_of<stapl::tuple<FlowTuples...>>
{
  using type = stapl::tuple<
                  ph_flow<
                    typename FlowTuples::output_t,
                    typename FlowTuples::input_t>...>;
};
...
struct x
{
  template<typename Skeleton, typename InPhs>
  flow_tuple<Skeleton, x, InPhs>
  operator=(flow_tuple<Skeleton, use_default, InPhs> const& f) const
  {
    return flow_tuple<Skeleton, x, InPhs>(f.get_skeleton());
  }

  template<typename Skeleton>
  flow_tuple<Skeleton, x, use_default>
  operator=(Skeleton const& s) const
  {
    static_assert(
      skeletons::is_skeleton<typename std::decay<Skeleton>::type>::value,
     "Flows can only be specified on skeletons or flow tuples");
    return flow_tuple<Skeleton, x, use_default>(s);
  }

  template<typename Placeholder>
  auto operator,(Placeholder const& p) const
  STAPL_AUTO_RETURN((
    ph_list<x,typename std::decay<Placeholder>::type>()
  ));
};
...
{
  template<typename Placeholder>
  auto operator,(Placeholder const& p)
  STAPL_AUTO_RETURN((
    ph_list<input,typename std::decay<Placeholder>::type>()
  ));
};
...
//////////////////////////////////////////////////////////////////////
/// @brief A trait to detect if a given type is a @c flow_tuple.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename T>
struct is_flow_tuple
  : std::false_type
{ };


template<typename... Ts>
struct is_flow_tuple<flow_tuple<Ts...>>
  : std::true_type
{ };


//////////////////////////////////////////////////////////////////////
/// @brief Gets the skeleton type from an argument type that is either
/// a skeleton itself or a flow tuple.
///
/// In addition to providing the type, it provides a call function
/// to extract the skeleton from the argument.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename S, bool = is_flow_tuple<typename std::decay<S>::type>::value>
struct get_skeleton_from
{
...
template<typename S>
struct get_skeleton_from<S, true>
{
  using type = typename std::decay<S>::type::skeleton_t;

  static type call(S const& s)
  {
    return s.get_skeleton();
  }
};


//////////////////////////////////////////////////////////////////////
/// @brief Converts a list of types of skeletons and @c flow_tuple into
/// a tuple of skeletons.
///
/// This is needed since the user may just provide the skeleton and
/// let the @c flow_tuple fill in the defaults.
///
/// @note Once Intel stops using gcc 4.7, this code can be
/// simplified to a type alias.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename... Ts>
struct get_skeletons_t
{
  using type = stapl::tuple<typename get_skeleton_from<Ts>::type...>;
};


namespace detail{
template<std::size_t... Is, typename... Ts>
typename get_skeletons_t<Ts...>::type
get_skeletons_from_helper(index_sequence<Is...>, Ts... t)
{
  return stapl::make_tuple(get_skeleton_from<Ts>::call(t)...);
}

} // namespace detail

//////////////////////////////////////////////////////////////////////
/// @brief Extracts a tuple of skeletons from a list of skeletons
/// and @c flow_tuple.
///
/// @param t  list of skeletons and flow_tuples.
/// @return   a tuple of the skeletons
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename... Ts>
typename get_skeletons_t<Ts...>::type
get_skeletons_from(Ts... t)
{
  using Indices = make_index_sequence<sizeof...(Ts)>;
  return detail::get_skeletons_from_helper(Indices(), t...);
}

//////////////////////////////////////////////////////////////////////
/// @brief Wraps a skeleton into a @c flow_tuple or leaves it alone
/// if it is already wrapped.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename S,
  bool = is_flow_tuple<typename std::decay<S>::type>::value>
struct to_flow_tuple
{
  using type = flow_tuple<typename std::decay<S>::type>;
...
template<typename S>
struct to_flow_tuple<S, true>
{
  using type = S;

  static type call(S const& s)
  {
    return type(s.get_skeleton());
  }
};


//////////////////////////////////////////////////////////////////////
/// @brief Converts a list of skeletons or partially specified
/// @c flow_tuples into a tuple of flow_tuples.
///
/// @note Intel compiler specifically fails with variadic alias here,
/// gcc-4.7 does not
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename... S>
struct to_flow_tuples_t
{
  using type = stapl::tuple<typename to_flow_tuple<S>::type...>;
};


namespace detail{
template<typename... S>
typename to_flow_tuples_t<S...>::type
make_flow_tuples_impl(S... s)
{
  return stapl::make_tuple(to_flow_tuple<S>::call(s)...) ;
}
} // namespace detail

//////////////////////////////////////////////////////////////////////
/// @brief Constructs a tuple of flow_tuples from
/// a list of skeletons or partially specified flow_tuples.
///
/// @param s  the list of skeletons or flow_tuples.
/// @return   a tuple of flow_tuples.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename... S>
typename to_flow_tuples_t<S...>::type
make_flow_tuples(S... s)
{
  return detail::make_flow_tuples_impl(s...);
}

namespace detail{
  template<typename Indices, typename PhFlows>
  struct fill_defaults_impl;


  template<std::size_t... Is, typename... PhFlows>
  struct fill_defaults_impl<index_sequence<Is...>, stapl::tuple<PhFlows...>>
  {
    using type = stapl::tuple<
      typename PhFlows::template fill_defaults_t<Is>...>;
  };
}

//////////////////////////////////////////////////////////////////////
/// @brief Fills in uses of stapl::use_default in a tuple of @c ph_flow
/// types.
///
/// @see ph_flow
/// @ingroup skeletonsFlowsInline
...
using fill_defaults_t = typename detail::fill_defaults_impl<
  make_index_sequence<stapl::tuple_size<PhFlows>::value>, PhFlows
>::type;
...
using get_index_of_ph =
  stapl::find_first_index<PhFlows, detail::writes_to_ph, Ph>;
...
///
/// @note This will fail with a static_assert if the key is not found.
///
/// @ingroup skeletonsFlowsInline
//////////////////////////////////////////////////////////////////////
template<typename Key, typename Map>
struct value_of_key
{ static_assert(true, "Couldn't find key in map."); };
...
struct compute_outputs_impl<Accum,
                            tuple<ph_flow<OutPh,ph_list<>>, PhFlows...>, I>
...
{
protected:
  using ph_flows = fill_defaults_t<PhFlows>;
  using outputs_map = compute_outputs_t<ph_flows>;
...
public:
  ...
  private:
    ...
    using get_ph_index = get_index_of_ph<Ph, ph_flows>;

    template<size_t N>
    using get_ph_of_index_t =
      typename tuple_element<N, ph_flows>::type::output_t;
  ...
  public:
    ...
    {
    public:
      template<typename Placeholder, typename = void>
    ...
    private:
      ...
      template<typename... InPhs>
      struct get_inflow_from_inputs<ph_list<InPhs...>>
      {
    ...
    public:
      ...
      struct get_in_flow_of
       : get_inflow_from_inputs<get_inputs_of_t<Ph, ph_flows>>
      { };


      template<size_t N>
      struct get_out_flow_of<input<N>, void>
      {
      ...
      template<typename Ph>
      struct get_out_flow_of<Ph, void>
      : tuple_element<get_ph_index<Ph>::value, skeletons_t>::type
        ::template out_port_type<typename get_in_flow_of<Ph>::type>
      { };
    ...
    : inner_port_types<In>::template get_out_flow_of<
      typename tuple_element<skeleton_size - 1, ph_flows>::type::output_t>
    { };
  ...
  private:
    //Tells if antyhing reads from a placeholder internally
    template<typename Ph>
    ...
    indexed_outputs_to_out_flow(size_t lid_offset, tuple<IndexedOutputs...>)
    {
      return stapl::make_tuple(
               std::get<IndexedOutputs::second_type::value>(
    ...
    {
      return stapl::make_tuple(std::get<static_cast<int>(N)>(in));
    }

    template<typename In, typename Ph>
    typename inner_port_types<In>::template get_out_flow_of<Ph>::type
    single_out_flow(In const&, size_t lid_offset, Ph, std::false_type)
    {
    ...
    in_flow_from_inputs(In const& in, size_t lid_offset, ph_list<InPhs...>)
    {
      return flows::concat(
               this->single_out_flow(in, lid_offset, InPhs(),
                 std::integral_constant<bool, is_input<InPhs>::value>())...
      );
    }
    ...
    {
      return stapl::make_tuple(flows::dontcare_flow());
    }
  ...
  public:
    ...
    using in_port_size = typename tuple_size<in_port_type>::type;
      ...
      std::integral_constant<int, I>, Last)
    {
      return this->in_flow_from_inputs<In, I>(
        out, lid_offset, typename tuple_element<I, ph_flows>::type::input_t()
      );
    }
    ...
    out_port(std::size_t lid_offset)
    {
      ...
      using input_t = typename inner_port_types<In>::template get_in_flow_of<
        typename tuple_element<last_index, ph_flows>::type::output_t
      >::type;

      return m_compose.template get_skeleton<last_index>().template
        out_port<input_t>(lid_offset);
    }

...
struct piped
{
  ...
  public:
    using in_port_size  = typename nested_p_type::in_port_size;
    ...
    in_flow(std::size_t iter_num, In const&, std::size_t lid_offset, Tag)
    {
      return m_repeat.template nested_out_port<In>(lid_offset, iter_num-1);
    }
...
struct output_from_all
{
  ...
  public:
    ...
    custom_flow(S const& skeleton, std::size_t lid_offset)
      : m_skeleton(skeleton),
    ...
    consume_from(Index const& index, F const& filter = F()) const
    {
      ...
      return stapl::get<0>(m_skeleton.template out_port<In>(offset)).
  ...
  public:
    using in_port_size = typename piped::port_types<Repeat>::in_port_size;
    ...
    out_port(std::size_t lid_offset)
    {
      return stapl::make_tuple(
               custom_flow<In, nested_p_type>(
                 this->m_repeat.nested_skeleton(), lid_offset
               ));
    }
  };
};
...
struct input_wrapper
{
    ...
    : public Flows::template port_types<Skeleton>
  {
    ...
    using in_port_size = typename base_flow_t::in_port_size;
    using in_port_type = decltype(
                           tuple_ops::transform(
                             std::declval<typename base_flow_t::in_port_type>(),
                             make_custom_flow()));
    ...
    port_types(Skeleton const& skeleton)
    ...
    ///        [custom_flow<Flow>]
    //////////////////////////////////////////////////////////////////////
    typename port_types<Skeleton>::in_port_type
    in_port(std::size_t lid_offset)
    {
      return tuple_ops::transform(base_flow_t::in_port(lid_offset),
                                  make_custom_flow());
    }
  };
};

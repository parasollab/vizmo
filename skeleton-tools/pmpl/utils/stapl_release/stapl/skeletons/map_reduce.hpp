...
map_reduce_impl(S const& skeleton, V&&... v)
{
  ...
  return algorithm_executor<C>().template execute<val_t>(
...
map_reduce_impl(S const& skeleton, Scheduler const& scheduler, V&&... v)
{
  ...
  using namespace skeletons;
  typedef typename df_stored_type<T>::type val_t;
  return algorithm_executor<C,stapl::use_default,Scheduler>(scheduler).
           template execute<val_t>(
             skeletons::sink_value<val_t>(
               compose(skeleton, broadcast_to_locs())
             ),
             std::forward<V>(v)...
...
///
/// @param use_default specifies that both data and skeleton need to be
///                  coarsened
...
///
/// @param no_coarsening specifies that neither data nor skeleton need
///                  to be coarsened
/// @param map_op    the operation to be applied in the first step on
///                  the given input(s)
/// @param reduce_op the operation to be applied on the results of the
...
template<typename MapOp, typename ReduceOp, typename ...V>
inline
typename stapl::result_of::map_reduce<
           skeletons::tags::no_coarsening, MapOp, ReduceOp, V...>::type
map_reduce(skeletons::tags::no_coarsening,
           MapOp const& map_op, ReduceOp const& reduce_op, V&&... v)
{
  typedef typename
    stapl::result_of::map_reduce<
...
///
/// @param with_coarsened_wf specifies that only data needs to be
///                  coarsened
/// @param map_op    the operation to be applied in the first step on
///                  the given input(s)
/// @param reduce_op the operation to be applied on the results of the
...
inline
typename stapl::result_of::map_reduce<
           skeletons::tags::with_coarsened_wf, MapOp, ReduceOp, V...>::type

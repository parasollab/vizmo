  ...
  : public decltype(
             skeletons::compose<Flows>(
               skeletons::reduce<Tag, stapl::use_default, Span>(
                 std::declval<Op>()),
               skeletons::broadcast_to_locs<Tag>()))
{
...
template <typename Tag,
          ...
          typename Span,
          typename Op>
using reduce_to_locs  = skeletons_impl::reduce_to_locs<
                          typename std::decay<Op>::type, Flows, Span, Tag>;
...
template <typename Tag   = stapl::use_default,
          ...
          typename Op>
result_of::reduce_to_locs<Tag, Flows, Span, Op>
reduce_to_locs(Op&& op)
{
  return result_of::reduce_to_locs<Tag, Flows, Span, Op>(
           std::forward<Op>(op));
}

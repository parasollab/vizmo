...
//////////////////////////////////////////////////////////////////////
template <typename Op, typename Span, bool pos_aware, typename Filter>
struct expand_from_pow_two
  : public decltype(
             skeletons::elem<Span>(
               skeletons::expand_from_pow_two_pd<pos_aware>(
           ...
           )
{
  ...
  expand_from_pow_two(Op const& op, Filter const& filter)
    : base_type(
        skeletons::elem<Span>(
          skeletons::expand_from_pow_two_pd<pos_aware>(op, filter)
        )
      )
  { }
...
struct expand_from_pow_two<Op, Span, pos_aware, stapl::use_default>
  : public decltype(
             skeletons::elem<Span>(
               skeletons::expand_from_pow_two_pd<pos_aware>(std::declval<Op>())
             )
           )
{
  using skeleton_tag_type = tags::expand_from_pow_two;
  ...
  expand_from_pow_two(Op const& op, stapl::use_default)
    : base_type(
        skeletons::elem<Span>(
          skeletons::expand_from_pow_two_pd<pos_aware>(op)
        )
      )
  { }
...
result_of::expand_from_pow_two<Span, pos_aware, Op, Filter>
expand_from_pow_two(Op&& op, Filter&& filter = Filter())
{
  return result_of::expand_from_pow_two<Span, pos_aware, Op, Filter>(
           std::forward<Op>(op),

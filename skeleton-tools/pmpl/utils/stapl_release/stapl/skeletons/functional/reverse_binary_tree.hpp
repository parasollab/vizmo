...
//////////////////////////////////////////////////////////////////////
template <typename Op, typename Flows, typename Span, typename Tag,
          typename Filter, bool isPositionAware>
struct reverse_binary_tree<Op, Flows, Span, Tag, true, Filter, isPositionAware>
  : public decltype(
             skeletons::reverse_tree<2, Flows, spans::reverse_tree<Span, Tag>>(
               skeletons::reverse_binary_tree_pd<Tag, isPositionAware>(
                 std::declval<Op>(), std::declval<Filter>())))
{
  ...
  reverse_binary_tree(Op const& op, Filter const& filter)
    : base_type(
        skeletons::reverse_tree<2, Flows, spans::reverse_tree<Span, Tag>>(
          skeletons::reverse_binary_tree_pd<Tag, isPositionAware>(op, filter))
      )
  { }
...
template <typename Op, typename Flows, typename Span, typename Tag,
          typename Filter, bool isPositionAware>
struct reverse_binary_tree<Op, Flows, Span, Tag, false, Filter, isPositionAware>
  : public decltype(
             skeletons::compose(
               skeletons_impl::reverse_binary_tree<
                 Op, Flows, spans::nearest_pow_2<Span>, Tag, true, Filter,
                 isPositionAware
               >(std::declval<Op>(), std::declval<Filter>()),
               skeletons::expand_from_pow_two<Span, isPositionAware>(
                 std::declval<Op>(), std::declval<Filter>())
             )
           )
{
  ...
  using base_type = decltype(
                      skeletons::compose(
                        skeletons_impl::reverse_binary_tree<
                          Op, Flows, spans::nearest_pow_2<Span>, Tag,
                          true, Filter, isPositionAware
                        >(std::declval<Op>(), std::declval<Filter>()),
                        skeletons::expand_from_pow_two<Span, isPositionAware>(
                          std::declval<Op>(), std::declval<Filter>())));
    ...
    : base_type(
        skeletons::compose(
          skeletons_impl::reverse_binary_tree<
            ...
            isPositionAware
          >(op, filter),
          skeletons::expand_from_pow_two<Span, isPositionAware>(op, filter)))
  { }
...
///               used in each @c reverse_binary_tree_pd
/// @param  tag   determines the type of span and @c reverse_binary_tree_pd
///               to be used
...
result_of::reverse_binary_tree<
  Tag, Flows, Span, isComplete,isPositionAware, Op, Filter>

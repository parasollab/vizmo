...
{
  skeletons::algorithm_executor<
    default_coarsener, stapl::use_default, Scheduler
  >().execute(skeletons::serial<sizeof...(V)>(std::forward<WF>(wf), num_sets),
              std::forward<V>(v)...);
}
...
{
  skeletons::algorithm_executor<
    null_coarsener, stapl::use_default, Scheduler
  >().execute(skeletons::serial<sizeof...(V)>(std::forward<WF>(wf), num_sets),
              std::forward<V>(v)...);
}

...
public:
  ...
  void
  spawn_element(std::size_t tid, WF const& wf, std::size_t no_succs,
                In&&... in)
  {
    typedef typename
    ...
    spawn_element<isResult>(
      ...
      tid, std::vector<std::size_t>(), wf, no_succs, std::forward<In>(in)...);
  }
  ...
  template <bool isResult, typename WF, typename ...In>
  void
  spawn_element(std::false_type,
                std::size_t tid,
                std::vector<std::size_t> const& notifications,
                WF const& wf, std::size_t no_succs, In&&... in)
  {
    typedef typename
      boost::result_of<WF(decltype(make_producer(in))...)>::type result_type;
...
private:
  ...
  make_producer(flows::view_producer_info<View> const& producer)
  {
  ...
  make_producer(flows::view_producer_info<proxy<T, A> > const& producer)
  {
  ...
  Element&
  make_producer(flows::reflexive_producer_info<Element> const& producer)
  {

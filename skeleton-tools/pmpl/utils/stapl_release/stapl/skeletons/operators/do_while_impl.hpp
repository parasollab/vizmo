...
public:
  ...
  using in_port_size        = typename Flows::template port_types<
                                do_while>::in_port_size;
...
public:
  ...
  {
    m_body_p.set_dimensions(spawner, view_domains);
    m_red_p.set_dimensions(spawner, view_domains);
    m_continuation_p.set_dimensions(spawner, view_domains);
    m_dims = 0;
  }
  ...
  out_port(std::size_t lid_offset) const
  {
    return Flows().template out_port<In>(*this, lid_offset);
  }
  ...
  auto nested_out_port(std::size_t lid_offset, std::size_t iter) const
    STAPL_AUTO_RETURN((
      this->get_nested_skeleton<Comp>().template out_port<In>(

...
private:
  ...
  {
    typedef typename Domain::gid_type        gid_t;
    typedef nd_linearize<gid_t, traversal_t> linearize_func_t;
  ...
  {
    using index_type = typename Partitions::index_type;

    auto const domain = part_loc.domain();

    // go through each local md entry
    domain_map(domain, [&](index_type const& idx)
    {
      auto const& md = part_loc[idx];
...
public:
                  ...
                  MDCache& global_md) const
  {
    constexpr bool is_infinite =
      has_finite_domain<typename Partitions::value_type>::value;
...
{
  typedef stapl::static_array<std::vector<typename Part::value_type> > type;
};
...
{
  typedef typename Part::value_type type;
};
...
private:
  ...
  multidimensional_rank(decomposed_partition_t const& part,
   ...
   -> decltype(nd_reverse_linearize<gid_type, traversal_t>(
                 part.dimensions())(rank)
               )
  {
    return nd_reverse_linearize<gid_type, traversal_t>(part.dimensions())(rank);
  }
  ...
  static void traverse_grid(decomposed_domain_t const& local_dom,
                            ...
                            global_md_storage_t& global_md)
  {
    using set_type = std::unordered_set<gid_type, stapl::hash<gid_type>>;
    using map_type = std::unordered_map<
      gid_type, cid_type, stapl::hash<gid_type>
    >;
    ...
    while (!points.empty())
    {
      ...
      auto q = vs_map_reduce(
        detail::find_entry_and_intersect<gid_type>(p),
        detail::element_wise_min<gid_type>(),
        index_bounds<gid_type>::invalid(), global_md, mds
      );
      ...
      detail::add_new_points<gid_type, cid_type>::apply(
...
public:
  ...
  /// @param GuideIndex Ignored. Alignment is guided by the spatial

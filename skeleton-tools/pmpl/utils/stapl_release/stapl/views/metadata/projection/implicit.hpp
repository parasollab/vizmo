...
struct implicit_metadata_projection
{
  ...
  typedef metadata_entry<
    domain_type,
    component_type,
    typename P::value_type::cid_type
  >                                                    dom_info_type;
  ...
  return_type* operator()(View* vw, P* part) const
  {
    ...
    for (size_t i = 0; i != nsubdomains; ++i)
    {
      ...
      if (!subdomain.empty()) {
        typedef
          std::pair<typename P::iterator, typename domain_type::index_type>
          loc_info;
        ...
        while (first != locality_info.end())
        {
          ...
          out_part->push_back_here(dom_info_type(
            typename P::value_type::cid_type(),
            coarsen_partition_impl::projection_construct_domain<domain_type>()(

...
#define CALC_size(z,i,var) \
  ((get<i>(dom.dimensions()) - get<i>(start)) / get<i>(step)) + \
  ((get<i>(dom.dimensions()) - get<i>(start)) % get<i>(step) ? 1 : 0) - 1
...
struct strided_domain_helper<this_iteration, S, D>
{
  ...
  {
#if this_iteration == 1
    typename D::index_type last(((dom.last()-start) / step)-1);
    return D(typename D::index_type(0),
      typename D::index_type(last +
        (dom.contains(start + last*step + step) ? 1 : 0))
    );
#else
  return D(typename D::index_type(
    BOOST_PP_ENUM(this_iteration, ZERO_dom, ~)),
    typename D::index_type(BOOST_PP_ENUM(this_iteration, CALC_size, ~))
  );
#endif
  }
};
...
public:
  ...
  BOOST_PP_CAT(f_stride,this_iteration)()
  { }
...
public:
  ...
  BOOST_PP_CAT(f_stride_inv,this_iteration)()
  { }
  ...
  {
#if this_iteration == 1
    return (idx - m_first_gid) % m_step == 0;
#else
    return BOOST_PP_REPEAT(this_iteration ,DEFINED_stride_op, ~);
#endif

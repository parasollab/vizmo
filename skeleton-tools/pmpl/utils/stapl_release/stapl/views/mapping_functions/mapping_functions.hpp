...
template<typename F, typename G,
         typename = make_index_sequence<
                      tuple_size<typename F::gid_type>::value>>
class mapfun_comp;
...
public:
  ...
  mapfun_comp(FParam&& f, GParam&& g)
    : F(std::forward<FParam>(f)), G(std::forward<GParam>(g))
  { }

  gid_type operator()(index_type const& x) const
  {
    return static_cast<F const&>(*this)(static_cast<G const&>(*this)(x));
  }
  ...
  {
    return static_cast<G const&>(*this).apply_get(
      outer_mapping<RefGetter, F>(
        get_reference, static_cast<F const&>(*this) ),
      indices... );
  }

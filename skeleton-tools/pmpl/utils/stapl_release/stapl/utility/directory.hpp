...
{
protected:
  bool requires_retry(Key const&) const
  {
    return false;
  }
}; // class non_migratable_base
...
protected:
  ...
  using registry_t =
    typename select_parameter<
      Registry,
      boost::unordered_map<
        Key, location_type, boost::hash<Key>,
        std::equal_to<Key>,
...
protected:
  using queue_t   = std::queue<request_t>;
...
protected:
  ...
  ///        entry is stored) is where @p pmf while be invoked.
  ...
  /// to the location where the can be serviced, forwarded, etc.
  ///
  /// @ingroup directory
  /// @sa directory_base::invoke_where
  //////////////////////////////////////////////////////////////////////
  template<typename... PMFArgs, typename... Args>
  void invoke_at_manager(Key const& key,
                         void (directory_base::* const pmf)(PMFArgs...),
                         Args&&... args)
  {
    ...
    {
      (this->*pmf)(std::forward<Args>(args)...);
      return;
    }
  ...
  template<typename... PMFArgs, typename... Args>
  void invoke_at_manager(Key const& key,
                         void (directory_base::* const pmf)(PMFArgs...) const,
                         Args&&... args) const
  {
    const std::pair<location_type, loc_qual> location = m_key_mapper(key);
...
protected:
                       ...
                       Functor&& f)
  {
    ...
    // cases could.
    //
    const typename registry_t::const_iterator rit = m_registry.find(key);
...
protected:
  ...
  ///        container framework.
  //////////////////////////////////////////////////////////////////////
  directory_base(mapper_t const& mapper,
                 registry_t const& registry)
...
public:
  ...
  /// registered.
  ///
  /// @param f Functor apply.  Unary operator, receives key as parameter.
...
public:
  ...
  ///   container framework.
  //////////////////////////////////////////////////////////////////////
  directory(typename base_t::mapper_t const& mapper,
            typename base_t::registry_t const& registry =
              typename base_t::registry_t())
    : base_t(mapper, registry)
  { }
...
public:
  ...
  directory(Function&& f,
            bool b_migration_enabled,
            bool b_enable_buffering)
    : base_t(std::forward<Function>(f), b_migration_enabled, b_enable_buffering)
  { }
...
private:
                    ...
                    Functor&& f)
  {
    ...
    const typename base_t::queues_t::iterator it = this->m_pending.find(key);
  ...
  //////////////////////////////////////////////////////////////////////
  /// @copydoc directory_base::invoke_at_manager
  //////////////////////////////////////////////////////////////////////
  template<typename... PMFArgs, typename... Args>
  void invoke_at_manager(Key const& key,
                         ...
                         Args&&... args)
  {
    const std::pair<location_type, loc_qual> location = this->m_key_mapper(key);
    ...
    if (location.second == LQ_CERTAIN)
    {
      base_t::transmitter_type::transmit(
        location.first, this->get_rmi_handle(), pmf,
        std::forward<Args>(args)...);
      return;
    }
    ...
    using mem_fun_t =
      void (directory::*)(Key const&, decltype(pmf), decltype(args)...);

    constexpr mem_fun_t mem_fun = &directory::invoke_at_manager;
                         ...
                         Args&&... args) const
  {
    ...
    using mem_fun_t =
      void (directory::*)(Key const&, decltype(pmf), decltype(args)...) const;

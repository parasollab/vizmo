...
balance(unsigned long int n, level lvl)
{
  ...
  auto map_factory =
    [n](location_type num_locs)
      { return balance_map<1, id_type, id_type>(n, num_locs); };

  return distribution_spec<>(
    new mapping_view_type(
      system_view(lvl),
      deferred_domain<indexed_domain<id_type>>(
        indexed_domain<id_type>(invalid_location_id),
        [](indexed_domain<id_type> const&, location_type num_locs)
          { return indexed_domain<id_type>(num_locs); }),
      identity_map<id_type, location_type>()),
    indexed_domain<id_type>(n),
...
balance(unsigned long int n, std::vector<location_type> const& locs)
{
  ...
  return distribution_spec<>(
    new mapping_view_type(
      system_view(locs),
      deferred_domain<indexed_domain<id_type>>(
        indexed_domain<id_type>(locs.size()),
        [](indexed_domain<id_type> const& dom, location_type)
          { return dom; }),
      identity_map<id_type, location_type>()),
    indexed_domain<id_type>(n),
  ...
  return distribution_spec<>(
    new mapping_view_type(
      ...
      deferred_domain<indexed_domain<id_type>>(
        ...
        [](indexed_domain<id_type> const& dom, location_type)
          { return dom; }),
      dist_spec_impl::cb_part_to_loc<View>(part_view)),
    indexed_domain<id_type>(min_max_gids.first, min_max_gids.second, true),
    dist_spec_impl::cb_gid_to_part<View>(part_view));
}
...
public:
  location_layout_reflector(LocDimensions const& loc_dims)
    : m_loc_dims(loc_dims)
  { }

...
struct size_wf
{
  ...
  result_type operator() (NativeView const& native_view, Count const& my_id,
                          WMap weight_map)
  {
    typename NativeView::iterator it = native_view.begin(),
                   ...
                   WMap weight_map)
  {
    ...
    if (num_elts_i_have > avg) {
      ...
      if (needed > 0) {  // if we still need more elements...
        ...
        typename NativeView::iterator it_begin = native_view.begin(), it;
        typename NativeView::iterator it_end = native_view.end();

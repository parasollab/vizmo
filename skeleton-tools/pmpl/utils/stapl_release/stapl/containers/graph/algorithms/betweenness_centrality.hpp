...
#include <stapl/containers/graph/algorithms/paradigms/level_sync_paradigm.hpp>
...
struct post_execute
{
  ...
  void operator() (GView& gv, size_t iterations)
  {
...
struct bc_mssp_wf
{
  ...
  bool operator()(Vertex v_vert, GraphVisitor graph_visitor)
  {
...
//////////////////////////////////////////////////////////////////////
template <typename GView>
void betweenness_centrality(GView& gv, size_t num_sources=0)
{
  ...
  for (size_t s = 0; s < gv_num_verts; s += num_sources) {
    ...
    dist_type level = level_sync_paradigm(bc_mssp_wf(),
                                          update_func(),
                                          post_execute(),
                                          gv);
    for (; level > 0; --level) {
...
//////////////////////////////////////////////////////////////////////
/// @brief Computes betweenness centrality value of unweighted, directed
/// graph using the hierarchical machine paradigm (@ref H_paradigm()).
/// @param gv View of graph on which centrality will be computed.
/// @param hv The hierarchical machine @ref graph_view over the input graph.
...
void betweenness_centrality(GView& gv, HView& hv, size_t num_sources=0)
{
  using namespace bc_algo_detail;
  ...
  for (size_t s = 0; s < gv_num_verts; s += num_sources) {
    ...
    dist_type level = h_paradigm(bc_mssp_wf(),
                                 update_func(),
                                 post_execute(),
                                 hv, gv);
    for (; level > 0; --level) {

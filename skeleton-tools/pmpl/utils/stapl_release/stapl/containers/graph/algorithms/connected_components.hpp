  ...
  result_type operator()(Vertex v, GraphVisitor graph_visitor)
  {
    if (v.property().active()) {
      graph_visitor.visit_all_edges(v,
        update_func<typename Vertex::vertex_descriptor>(v.property().cc()));
...
size_t connected_components(GView& g, size_t k=0)
{
  ...
  return graph_paradigm(cc_map_wf(), update_func_t(), g, k);
}


//////////////////////////////////////////////////////////////////////
/// @brief A Parallel Connected Components (CC) Algorithm using the
...
size_t connected_components_h(GView& g, HView& h)
{
  using namespace cc_algo_detail;
  ...
  map_func(init_cc_wf(), g);
  return h_paradigm(cc_map_wf(), update_func_t(), h, g);
}
...
size_t connected_components_h(GView& g, HView& h, HubsView& hubs)
{
  ...
  map_func(init_cc_wf(), g);
  return h_hubs_paradigm(cc_map_wf(), update_func_t(), vp_reducer(),
                         h, hubs, g);
}

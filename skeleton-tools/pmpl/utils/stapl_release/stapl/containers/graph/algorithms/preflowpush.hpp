...
struct print_flows
{
  ...
  result_type operator()(Vertex v)
  {
    ...
    for (auto ii = v.begin(), ee = v.end(); ii != ee; ++ii)
      print_edge(v.descriptor(), (*ii));
  }
};
...
struct add_backward_edges
{
  ...
  {
    for (auto ii = v.begin(), ee = v.end(); ii != ee; ++ii) {
      gView.add_edge((*ii).target(), v.descriptor(),
...
public:
  ...
  result_type operator ()(Vertex target) const
  {
    ...
    auto ii = target.begin(), ee = target.end();
    for (; ii != ee; ++ii)
    {
  ...
  result_type operator()(Vertex v, GraphVisitor graph_visitor) const
  {
    ...
    if (v.descriptor() == m_source) {
      ...
      for (auto ii = v.begin(), ee = v.end(); ii != ee; ++ii) {
        ...
        auto amount = (*ii).property().rc();
        (*ii).property().dec_rc(amount);

        auto edge_tg = (*ii).target();
...
void relabel(Vertex v, int num_vertices)
{
  ...
  for (auto ii = v.begin(), ee = v.end(); ii != ee; ++ii, ++current_edge) {
...
struct gr_wf
{
  ...
  result_type operator()(Vertex v, GraphVisitor graph_visitor)
  {
    ...
    if (v.property().height() == graph_visitor.level()) {

      for (auto ii = v.begin(), ee = v.end(); ii != ee; ++ii) {
        if ((*ii).property().backward() && (*ii).property().rc() == 0) {
          graph_visitor.visit((*ii).target(),

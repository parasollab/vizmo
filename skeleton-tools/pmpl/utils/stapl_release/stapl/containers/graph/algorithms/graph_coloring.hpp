                         ...
                         Array& hashmap_array) const
  {
    ...
    for (; it!=end_it; ++it) {
      ...
      if (responsible_for_conflict) {
        typename const_vertex_iterator::reference::const_adj_edge_iterator
          edge_it = (*it).begin(), edge_end_it = (*it).end();
        for (; edge_it!=edge_end_it; ++edge_it) {
          size_t target = (*edge_it).target();
                 ...
                 ColorMap& color_map)
{
  ...
  //Call graph coloring with an undirected graph
  color_graph(undir_view, color_map);
}

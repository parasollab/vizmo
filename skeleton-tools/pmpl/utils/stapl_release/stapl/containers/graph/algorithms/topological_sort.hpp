...
struct increment_in_degree
{
  ...
  {
    graph_visitor.visit_all_edges(v, inc_preds_wf());
    return false;
  }
};
...
struct topological_sort_map_wf
{
  ...
  result_type operator()(Vertex v, GraphVisitor graph_visitor) const
  {
    if (v.property().preds() == 0) {
      v.property().preds(-1);
      graph_visitor.visit_all_edges(v, update_func(v.property().rank()+1));
      return true;
    }
    return false;
  }
...
void topological_sort(GraphView& gvw, size_t k=0)
{
  ...
  //vertex by one
  level_sync_paradigm(increment_in_degree(), inc_preds_wf(), gvw);

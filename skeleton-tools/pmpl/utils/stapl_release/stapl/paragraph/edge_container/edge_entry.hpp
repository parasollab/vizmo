...
public:
  remote_notifier(Notifier notifier, size_t loc)
    : Notifier(std::move(notifier)), m_loc(loc)
  { }
  ...
  //////////////////////////////////////////////////////////////////////
  /// @brief Wraps function operator of @p Notifier, passing member
  /// @p m_loc appropriately in the parameter list.
  ///
  ...
  {
    Notifier::operator()(ct, m_loc, tid, version, b_migrate, b_moveable);
  }
}; // class remote_notifier
...
public:
                     ...
                     Filter const& filter)
  {
    ...
    auto ret_val =
      filtered_versions.add_notifier(
        ...
        [full_ptr]()->stored_value_t const& { return full_ptr->value(); },
                      ...
                      const bool b_upgrade)
  {
    ...
    m_remote_version.add_notifier(
      ...
      [&full_version]()->edge_version_storage<stored_value_t> const&
        { return full_version.version_storage(); },
                        ...
                        bool b_caller_is_migration   = false)
  {
    ...
    full_version_entry.set_value(
      wrapper, executor, b_migration_enabled, ct.is_persistent());
    ...
    if (b_caller_is_set_element)
    {
      ...
      m_remote_version.set_value(
        ct, tid(),
        [&full_version_entry]()->edge_version_storage<stored_value_t> const&
          { return full_version_entry.version_storage(); },
    ...
    if (filtered_iter != m_versions.end())
    {
      down_cast<filtered_edge_versions_t&>(*filtered_iter).set_value_full(
        full_version_entry.value(), executor,
  ...
  void
  set_value_impl(ValueParam&& val, edge_container& ct, executor_base& executor,
                 ...
                 bool b_expect_exist, std::true_type)
  {
    ...
    {
      full_version_entry.set_value(
        std::forward<ValueParam>(val), executor,
        b_migration_enabled, is_persistent());
    }
    else
    {
      full_version_entry.set_value(
        ...
        executor, b_migration_enabled, is_persistent());
    }
                 ...
                 bool b_expect_exist, std::false_type)
  {
    ...
    get_filtered_versions(b_expect_exist). template set_value<Filter>(
      std::forward<ValueParam>(val), version_id, executor,
                 ...
                 bool b_caller_is_migration   = false)
  {
    ...
    this->set_value_impl<Filter>(
      std::forward<ValueParam>(val), ct, executor, version_id,

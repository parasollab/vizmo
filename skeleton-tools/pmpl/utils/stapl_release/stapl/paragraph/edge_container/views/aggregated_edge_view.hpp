...
{
private:
  /// @brief Used during release of lazy_reference from @ref edge_container
  task_graph*    m_tg_ptr;
...
public:
  ...
  aggregated_edge_subview(task_graph* tg_ptr, ProxyContainer&& ct)
    : m_tg_ptr(tg_ptr), m_container(ct)
  { }
  ...
  {
    stapl_assert(idx < m_container.size(),
      "aggregated_edge_subview::operator[]: out of bounds");

    return m_container[idx].get_reference();
  }
  ...
  ~aggregated_edge_subview()
  {
    for (auto&& elem : m_container)
      elem.release(tg_callback(m_tg_ptr));
  }
}; // class aggregated_edge_subview
...
///
/// @todo Empty base optimization of m_filter to save memory.
///
...
public:
  ...
  explicit
  aggregated_edge_view(task_graph* tg_ptr, OptionalFilterParam&&... filter)
  ...
  {
    return reference(m_tg_ptr, std::move(m_reference_param));
  }
  ...
  {
    for (auto&& elem : m_reference_param)
      elem.release(tg_callback(m_tg_ptr));
  }
}; // class aggregated_edge_view

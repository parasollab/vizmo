...
class edge_container final
  : public directory<
      std::size_t, try_transmitter, use_default, use_default, true>,
...
private:
  ...
  using directory_t =
    directory<index_type, try_transmitter, use_default, use_default, true>;
  ...
  /// @brief Used to guard reentrant calls into @p try_delete.
  bool                                                   m_b_delete_in_process;
...
public:
  ...
  template<typename T>
  detail::edge_entry<T>&
  add_producer(size_t tid, size_t consumer_cnt, bool b_migrated = false)
  {
...
public:
  ...
  //////////////////////////////////////////////////////////////////////
  /// @brief Public method called by the PARAGRAPH to request invocation of the
  ...
  //////////////////////////////////////////////////////////////////////
  /// @brief Receive notification that associated @ref task_graph
  /// is in the process of destruction and has no more need for this object.
  ///
  /// Conditionally attempts deletion if reference count reaches zero
  /// and there is no reason to delay the event.
  //////////////////////////////////////////////////////////////////////
  void release(void)
  {
    if (!m_b_delete_in_process)
      this->try_delete();
  }
...
private:
  ...
  /// PARAGRAPH. Termination detection (at least on the location) has succeeded.
  ///
  /// It is possible that the edge_container made need to stay alive longer
  /// if there are pending consumption requests from remote locations.
  //////////////////////////////////////////////////////////////////////
  void try_delete(void)
  {
    if (!m_b_delay_delete && !pending_consumers())
    {
      m_b_delete_in_process = true;
  ...
  {
    if (m_b_orphaned && !pending_consumers())
    {
      stapl_assert(!m_b_delete_in_process && !m_b_tg_persistent,

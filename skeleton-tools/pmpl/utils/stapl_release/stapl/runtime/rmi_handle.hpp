                                        ...
                                        const unsigned int flags)
{
  ...
  this->m_flags = flags;
  this->m_nlocs = ctx.get_gang_md().size();

  if ((this->m_nlocs==1) && ((flags & allow_try_rmi)==0)) {
    static_cast<rmi_handle_info&>(*this) = m_location->object_register(t, true);
  }
  else {
    ctx.flush_requests(); // epoch will change, flush aggregated requests
    static_cast<rmi_handle_info&>(*this) =
      m_location->object_register(t, false);
  }
}
...
inline void rmi_handle::unregister_object(void)
{
  ...
  m_location->object_unregister(*this);
  m_location = nullptr;
...
T& retrieve_object(Handle&& h, location_md& l) noexcept
{
  ...
  T* const t = l.object_get<T>(h.internal_handle(), h.get_registration_epoch());
  STAPL_RUNTIME_ASSERT_MSG(t, "p_object does not exist.");
  return *t;
}
...
T* try_retrieve_object(Handle&& h, location_md* const l) noexcept
{
  ...
  return l->object_get<T>(h.internal_handle(), h.get_registration_epoch());
}
...
public:
  ...
  { return l.object_get<T>(m_handle); }
};
...
public:
  ...
  template<typename T>
  T* get(location_md& l) const noexcept
  { return l.object_get<T>(m_handle, m_registration_epoch); }
};

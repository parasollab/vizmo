...
public:
  ...
  T& reset(U&& u)
  {
    ...
    return ctx.get_location_md().lss_reset<T>(m_id, std::forward<U>(u));
  }
  ...
  {
    auto& ctx = runtime::this_context::get();
    return ctx.get_location_md().lss_get<T>(m_id, m_creator_f);
  }
  ...
  T* try_get(void) const
  {
    ...
    return (!ctx ? nullptr : ctx->get_location_md().lss_try_get<T>(m_id));
  }
  ...
  void destroy(void)
  {
    auto* const ctx = runtime::this_context::try_get();
    if (ctx)
      ctx->get_location_md().lss_destroy(m_id);
  }
};

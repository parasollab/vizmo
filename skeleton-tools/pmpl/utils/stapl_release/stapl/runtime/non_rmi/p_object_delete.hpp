...
public:
  ...
  {
    T& c = retrieve_object<T>(handle_storage_type::get(this),
                              ctx.get_location_md());
...
public:
  ...
  void operator()(rmi_handle::reference const& h) const
  {
    ...
    typedef delete_object_request<T, deleter_type> request_type;
    const std::size_t size = request_type::expected_size(h, *this);

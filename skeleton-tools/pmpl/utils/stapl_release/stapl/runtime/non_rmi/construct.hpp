...
{
  rpc<
    decltype(&construct_all_impl<decltype(c), decltype(f)>),
    &construct_all_impl<decltype(c), decltype(f)>
...
{
#ifdef BOOST_INTEL
  // Intel 16 does not produce the right type through decltype
  using callback_type = typename std::decay<Callback>::type const&;
  using function_type = typename std::decay<Function>::type const&;
#else
  using callback_type = decltype(c);
  using function_type = decltype(f);
#endif
  rpc<
    decltype(&construct_range_impl<callback_type, T, function_type>),
    &construct_range_impl<callback_type, T, function_type>
  >(std::forward<ProcessRange>(pids),
    std::forward<Callback>(c),
  ...
  if (r.size()==1) {
    ...
    if (!g) {
      ...
      if (pid==runqueue::get_process_id())
        STAPL_RUNTIME_ERROR("Gang metadata should be available at the owner.");
      rpc<
        decltype(&fwd_construct_range_impl<decltype(c), T, decltype(f)>),
        &fwd_construct_range_impl<decltype(c), T, decltype(f)>
      >(pid,
        std::forward<Callback>(c),

...
#include "utility/object_type_registry.hpp"
...
//////////////////////////////////////////////////////////////////////
/// @brief Contains all the metadata for describing a location.
///
/// The metadata includes the fence metadata for the intragang requests of the
/// contexts of the associated location, the objects that are registered through
/// @ref rmi_handle or @ref p_object, any @ref location_specific_storage objects
/// and the @ref runqueue to schedule requests.
///
/// All @ref location_md objects are reference counted and are destroyed when
/// this count reaches @c 0. They notify their respective @ref gang_md object
...
private:
  ...
  object_type_registry           m_snapshot;
...
public:
  ...
  //////////////////////////////////////////////////////////////////////
  void set_executor(std::unique_ptr<executor_base> ex)
  {
    if (m_executor) {
      if (!m_executor->empty()) {
  ...
  executor_base* try_get_executor(void)
...
// ----------------------------------------------------------------------------
  ...
  //////////////////////////////////////////////////////////////////////
  /// @brief Registers @p p that points to a distributed object and returns
  ///        a new handle for it.
  ///
  /// If @p avoid_registry is @c true, then the object will not be inserted in a
  /// registry and the epoch will not be increased.
  ///
  /// This function will increase the reference count of this @ref location_md
  /// object.
  //////////////////////////////////////////////////////////////////////
  template<typename T>
  rmi_handle_info object_register(T* const p, const bool avoid_registry)
  {
    // increase ref count to protect metadata
    if (++m_num_objects==1)
      this->add_ref();

    if (get_debug_level() > 0) {
      if (!m_snapshot.insert(p))
        STAPL_RUNTIME_ERROR("p_object is already registered.");
    }

    const auto gid = get_gang_md().get_id();

    if (avoid_registry) {
      // registry can be avoided, do not advance epoch
      const auto e = get_epoch();
      return rmi_handle_info{p, e, gid};
    }

    // object has to be registered normally
    const auto e = (get_epoch() + 1); // get next epoch
    const rmi_handle_info h{m_registry.object_register(p, e), e, gid};
    advance_epoch();
    return h;
  }
  ...
  ///
  /// This function will decrease the reference count of the @ref location_md
  /// object. If the reference count is 0, this @ref location_md will delete
  /// itself.
  //////////////////////////////////////////////////////////////////////
  void object_unregister(rmi_handle_info const& h)
  {
    STAPL_RUNTIME_ASSERT(h.get_gang_id()==get_gang_md().get_id());

    auto const& ih = h.internal_handle();

    switch (ih.get_type()) {
      case object_virtual_address::DIRECT:
        if (get_debug_level() > 0) {
          if (!m_snapshot.erase(ih.direct()))
            STAPL_RUNTIME_ERROR("p_object is not registered.");
        }
        break;

      case object_virtual_address::INDIRECT: {
        auto const& t = ih.indirect();
        if (get_debug_level() > 0) {
          if (!m_snapshot.erase(m_registry.object_get(t)))
            STAPL_RUNTIME_ERROR("p_object is not registered.");
        }
        const bool immediate_unregistration = is_single();
        m_registry.object_unregister(t, immediate_unregistration);
      } break;
    ...
    // decrease ref count, metadata may get destroyed
    if (--m_num_objects==0)
      this->release();
  }

  template<typename T>
  T* object_get(object_virtual_address const& h) const
  {
    void* p = nullptr;
    ...
    switch (h.get_type()) {
      ...
      case object_virtual_address::INDIRECT:
        p = m_registry.object_get(h.indirect());
        if (!p)
          return nullptr;
        break;
    ...
    if (get_debug_level() > 0) {
      if (!m_snapshot.check_type<T>(p))
        STAPL_RUNTIME_ERROR("p_object retrieved with wrong type.");
    }
  ...
  template<typename T>
  T* object_get(object_virtual_address const& h, const epoch_type e) const
  {
    void* p = nullptr;
    ...
    switch (h.get_type()) {
      ...
      case object_virtual_address::INDIRECT:
        p = m_registry.object_get(h.indirect(), e);
        if (!p)
          return nullptr;
        break;
...
// ----------------------------------------------------------------------------
  ...
  T& lss_get(void* addr, CreatorFunction&& f)
  {
    using stored_type = lss_element<T>;

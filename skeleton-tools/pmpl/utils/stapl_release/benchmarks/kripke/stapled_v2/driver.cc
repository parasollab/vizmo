...
void usage(void){
  ...
  if(myid == 0){
    ...
    printf("                         Must be a multiple of 8, and divide evenly the number\n");
...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  if (myid == 0) {
    ...
    printf("ZonesperSet :          %d x %d x %d\n", zones_per_set[0], zones_per_set[1], zones_per_set[2]);
  ...
  if(test){
    ...
    snprintf(line, 2048, "RUN: ntasks=%d nthreads=%d nestid=%d nest=%s D=%-3d d=%-3d dirs=%d G=%-3d g=%-3d grps=%d Solve=%-8.4lf Sweep=%-8.4lf LTimes=%-8.4lf LPlusTimes=%-8.4lf\n",
      ...
      nesting.c_str(),
      vars.num_dirsets,
      vars.num_dirs_per_dirset,
      vars.num_directions,
      vars.num_groupsets,
      vars.num_groups_per_groupset,
      vars.num_groups,

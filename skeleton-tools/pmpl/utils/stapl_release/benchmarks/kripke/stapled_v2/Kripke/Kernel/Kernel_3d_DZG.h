...
struct Kernel_3d_DZG_dir_loop_wf
{
private:
  // Grid is needed to access metadata (e.g. gd_sets) stored on it.
  Grid_Data<NEST_DZG>* grid_data;
...
public:
  ...
  Kernel_3d_DZG_dir_loop_wf(Grid_Data<NEST_DZG>* data, Group_Dir_Set gdset,
    std::pair<Grid_Sweep_Block, Grid_Sweep_Block> grid_extents)
    : grid_data(data), gd_set(gdset), extents(grid_extents)
  { }

  template<typename DirectionId, typename PsiView, typename RhsView,
           typename SigtView, typename IPlane, typename JPlane, typename KPlane>
  result_type
  operator()(DirectionId d, PsiView& psi, RhsView& rhs, SigtView& sigt,
             IPlane const& i_plane_in, JPlane const& j_plane_in,
             KPlane const& k_plane_in)
  {
    // Index in to psi and rhs is (i, j, k, g)
    ...
    // Because our results will be merged with results from other directions
    // we have to zero out the plane information for those directions.
    for (int curr_d = 0; curr_d < num_directions; ++curr_d)
    {
      if (curr_d != d)
      {
              ...
              kcs += extents.second.inc_k)
          {
                ...
                jcs += extents.second.inc_j)
            {
              int i_plane_idx = I_PLANE_INDEX(jcs, kcs) * num_directions
                * num_groups + curr_d * num_groups;
              i_plane[i_plane_idx+group] = 0.;
                  ...
                  ics += extents.second.inc_i)
              {
                ...
                int k_plane_idx = K_PLANE_INDEX(ics, jcs) * num_directions
                  * num_groups + curr_d * num_groups;
                j_plane[j_plane_idx+group] = 0.;
                k_plane[k_plane_idx+group] = 0.;
              }
            }
          }
        }
    ...
    std::get<3>(sigt_idx) = 0; // sigt_idx has a single element in dir
        ...
        k += extents.first.inc_k, kcs += extents.second.inc_k)
    {
          ...
          j += extents.first.inc_j, jcs += extents.second.inc_j)
      {
        ...
        double * psi_lf_d_z = &i_plane[i_plane_idx];
            ...
            i += extents.first.inc_i, ics += extents.second.inc_i)
        {
          ...
          int j_plane_idx = J_PLANE_INDEX(ics, kcs) * num_directions
            * num_groups + d * num_groups;
          double * psi_fr_d_z = &j_plane[j_plane_idx];

          int k_plane_idx = K_PLANE_INDEX(ics, jcs) * num_directions
            * num_groups + d * num_groups;
          double * psi_bo_d_z = &k_plane[k_plane_idx];

          for (int group = 0; group < num_groups; ++group)
          {
            std::get<3>(psi_idx) = group;
            std::get<3>(rhs_idx) = group;
            std::get<4>(sigt_idx) = group;

            /* Calculate new zonal flux */
            double psi_d_z_g = (rhs[rhs_idx]
                + psi_lf_d_z[group] * xcos_dxi
                + psi_fr_d_z[group] * ycos_dyj
                + psi_bo_d_z[group] * zcos_dzk)
              / (xcos_dxi + ycos_dyj + zcos_dzk + sigt[sigt_idx]);
            psi[psi_idx] = psi_d_z_g;

            /* Apply diamond-difference relationships */
            psi_d_z_g *= 2.0;
            psi_lf_d_z[group] = psi_d_z_g - psi_lf_d_z[group];
            psi_fr_d_z[group] = psi_d_z_g - psi_fr_d_z[group];
            psi_bo_d_z[group] = psi_d_z_g - psi_bo_d_z[group];
          }
        }
      }
    }
...
template<typename SigtView, typename PsiView, typename RhsView,
         typename IPlane, typename JPlane, typename KPlane>
Kernel::result_type
Kernel_3d_DZG::operator()(SigtView& sigt, PsiView& psi, RhsView& rhs,
                          IPlane const& i_plane_in, JPlane const& j_plane_in,
                          KPlane const& k_plane_in)
{
  ...
  Kernel_3d_DZG_dir_loop_wf<result_type>
    dir_loop_body(grid_data, gd_set, extents);
  ...
  // direction dimension.  The psi and rhs views both have elements in the
  // direction dimension equal to the number of directions in the direction set.
  result_type result = stapl::map_reduce<stapl::skeletons::tags::no_coarsening>(
    dir_loop_body, reduce_planes<result_type>(),
    stapl::counting_view<int>(num_directions),
    stapl::make_slices_view<3>(psi), stapl::make_slices_view<3>(rhs),

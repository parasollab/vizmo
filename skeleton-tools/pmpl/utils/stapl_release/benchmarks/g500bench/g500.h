                              ...
                              bool remove_multiedges = false)
{
  ...
  if (b) {
    auto edge_view = make_edge_list_view(nedges, nedges_64, edges);
    free(edges);
    stapl::transform_edge_list(edge_view, graph_vw);
  }
  ...
  if (remove_multiedges) {
    map_func(remove_multiedges_wf(), graph_vw);
  }

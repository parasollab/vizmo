...
{
  const gang_id gid = registry.reserve_id();
...
void gang_md_registry::unregister_gang_md(gang_md const& g)
{
  ...
  if (gid==0) {
    ...
    if (!t.is_root()) {
      ...
      rpc<decltype(&release_gang_md), &release_gang_md>(t.parent_id(), gid);
    }
  }
}
...
void gang_md_registry::erase_fence_md(fence_md* const p)
{
  ...
  const gang_md::id gid  = p->get_gang_id();
  gang_md* const g       = try_get(gid);
  const process_id owner = id_owner(gid);
  ...
  if (g) {
    ...
    rpc<
      decltype(&gang_md::update_fence_md), &gang_md::update_fence_md
    >(owner, gid, p->retrieve());
  }

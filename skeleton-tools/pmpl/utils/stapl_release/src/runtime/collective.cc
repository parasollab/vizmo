...
collective& collective::create(collective::id const& id, topology const& t)
{
...
// Creates a new collective handle or returns an already existing one
static collective& collective_create(collective::id const& id)
{
  std::lock_guard<std::mutex> lock{collective_registry_mtx};
...
// Notifies that m has arrived from a child process
void collective::notify_arrival(collective::id const& id, message_ptr m)
{
  auto& c = collective_create(id);
  STAPL_RUNTIME_ASSERT(!c.get_topology().is_leaf());
  c.notify_arrival(std::move(m));
}

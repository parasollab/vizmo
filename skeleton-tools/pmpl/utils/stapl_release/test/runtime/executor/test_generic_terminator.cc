...
public:
  ...
  {
    typedef terminator<int, std::plus<unsigned int>> terminator_type;

    terminator_type t{std::plus<unsigned int>{},
                      [this] { return this->pending(); }};
  ...
  {
    typedef terminator<unsigned int, std::plus<unsigned int>> terminator_type;
    ...
    for (auto i = 0u; i < NUM_TERMINATORS; ++i) {
      terminators.emplace_back(std::plus<unsigned int>{}, [] { return 0; });

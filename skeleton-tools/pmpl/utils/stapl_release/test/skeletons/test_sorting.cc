...
void test_sorting(std::size_t n)
{
  ...
  debug_filename << "bitonic_" << n;
  exec_t exec(graphviz_env(debug_filename.str()));
#endif

  exec.execute(skeletons::sink<value_t>(bitonic_sort_pattern),
  ...
  bool is_valid = stapl::equal(input_view, output_view);

  do_once([is_valid](void) {
    STAPL_TEST_REPORT(is_valid, "bitonic_sort");
  });
#endif
  ...
  // define global oddeven merge sort pattern
  auto odd_even_merge_sort =
         skeletons::repeat(
...
#ifdef GRAPHVIZ_OUTPUT
  debug_filename << "oddevenmerge_" << n;
  exec.set_extra_env(graphviz_env(debug_filename.str()));
#endif

  exec.execute(skeletons::sink<value_t>(odd_even_merge_sort),
               input_view, output_view);
...
#ifdef VALIDATE_RESULTS
  stapl::sort(input_view);
  is_valid = stapl::equal(input_view, output_view);

  do_once([&is_valid](void) {
    STAPL_TEST_REPORT(is_valid, "odd_even_merge_sort");
  });
#endif
}

...
stapl::exit_code stapl_main(int argc, char** argv)
{
  ...
  value_type r1 = exec.execute<value_type>(sink_value<value_type>(s1), vc);
  value_type r2 = exec.execute<value_type>(sink_value<value_type>(s2), vc);
  value_type r3 = exec.execute<value_type>(sink_value<value_type>(s3), vc);

  // increment each value in the nested container by m
  exec.execute(map(map(increment<value_type>(10))), vc);

  value_type r4 = exec.execute<value_type>(sink_value<value_type>(s1), vc);
  ...
  STAPL_TEST_REPORT(
    (r4-r) == 10 * (m*n), "Testing map(map(increment)");

...
struct partition_functor
{
  ...
  { std::sort(b.begin(), b.end(), stapl::algo_details::comparator_wrapper<Compare>(m_comp)); }
...
struct check_functor1
{
  ...
  check_functor1(size_t sz, partitions_vt& result, data_vt& data_cp_v,
                 comparator_t comparator, std::string str)
    : m_size(sz), m_result(result), m_data_cp_v(data_cp_v),
      m_comparator(comparator), m_str(str)
  { }
  ...
  void operator()(void) const
  {
    ...
    if (std::equal(data_concat_v.begin(), data_concat_v.end(),
      ...
      std::cout << "n_partition validator | " << m_str
                << " shared splitters PASSED\n";
    else
      std::cout << "n_partition validator | " << m_str
                << " shared splitters FAILED\n";
  }
};
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  */
  ...
  stapl::do_once(check_functor1(data_second_v.size(), result_second,
                                data_cp_second_v, comparator, "explicit"));

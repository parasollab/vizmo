...
eval $run_command ./test_summa 4 9 11
if test $? -eq 0
then
    PASSED_TEST=`expr $PASSED_TEST + 1`;
else
    echo "ERROR:: while testing SUMMA with rectangle matrices."
fi

eval $run_command ./test_summa 6 6 6
if test $? -eq 0
then
    PASSED_TEST=`expr $PASSED_TEST + 1`;
else
    echo "ERROR:: while testing SUMMA with square matrices."
fi

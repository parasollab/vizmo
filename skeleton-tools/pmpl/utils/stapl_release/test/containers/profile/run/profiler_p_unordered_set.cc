...
public:
  ...
  /// @param in The input
  //////////////////////////////////////////////////////////////////////////////
  us_insert_profiler(std::string pcname, pC* pc, Input* in,
  ...
  void run()
  {
    for (m_it = m_ic->begin();m_it!=m_it_end;++m_it)
      this->m_pc->insert(value_type(*m_it));
    rmi_fence();
  }
...
public:
  ...
  /// @param in The input
  //////////////////////////////////////////////////////////////////////////////
  us_find_profiler(std::string pcname, pC* pc, Input* in,
  ...
  {
    for (m_it=m_ic->begin();m_it!=m_it_end;++m_it)
      typename pC::iterator val = (*(this->m_pc)).find(*m_it);
    rmi_fence();
  }
...
public:
  ...
  /// @param in The input
  //////////////////////////////////////////////////////////////////////////////
  us_erase_profiler(std::string pcname, pC* pc, Input* in,
                    int argc = 0, char **argv = 0)
  : base_type(pc, pcname+"_erase", argc, argv),
    m_ic(in)
  {
    m_it = m_ic->begin(); m_it_end = m_ic->end();
    this->n_times = m_ic->size();
...
{
  us_insert_profiler<pC, SC, counter_type>
    aip(pcname, &p, &words, argc, argv);
  aip.collect_profile();
  aip.report();

  us_find_profiler<pC, SC, counter_type>
    afp(pcname, &p, &words, argc, argv);
  afp.collect_profile();
  afp.report();

  us_erase_profiler<pC, SC, counter_type>
...
void evaluate_string(size_t N, int argc, char** argv)
{
  ...
  typedef stapl::unordered_set<MYKEY, my_hash<MYKEY>,
                               std::equal_to<MYKEY>
                              >                              unordered_set_type;

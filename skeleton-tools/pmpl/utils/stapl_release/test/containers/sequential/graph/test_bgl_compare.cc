...
// the weight with colors;
class my_vertex_property{
  size_t m_flux;
  typedef graph_color<size_t>::value_type color_value_type;
  color_value_type m_color;
public:
  my_vertex_property(size_t i=999):m_flux(i),m_color(graph_color<size_t>::white()) {}

  void set_color(color_value_type _c){ m_color = _c; }
  color_value_type get_color() const { return m_color; }

  size_t flux(){ return m_flux;}
};

struct my_vertex_property_bgl_composed {
  size_t m_flux;
  boost::default_color_type color;
  my_vertex_property_bgl_composed(size_t i=999):m_flux(i),color(color_traits<boost::default_color_type>::white()) {}
  size_t flux(){ return m_flux;}
};

struct my_vertex_property_bgl {
  size_t m_flux;
  my_vertex_property_bgl(size_t i=999) : m_flux(i) {}
  size_t flux(){ return m_flux;}
};
...
void  add_vertices_stapl(Graph& g){
  for(size_t i=0;i<N;++i) g.add_vertex();
}
...
void  add_edges_stapl(Graph& g){
  for(size_t i=0;i<EF*N;++i) g.add_edge(rand() % N, rand() % N);
}

template <class Graph>
void  add_edges_bgl(Graph& g){
  for(size_t i=0;i<EF*N;++i) add_edge(rand() % N, rand() % N, g);
}

//////////////////////////////////////////////// delete_edge
template <class Graph>
void  delete_edges_stapl(Graph& g){
  for(size_t i=0;i<EF*N;++i) g.delete_edge(rand() % N, rand() % N);
}

template <class Graph>
void  delete_edges_bgl(Graph& g){
  for(size_t i=0;i<EF*N;++i) remove_edge(rand() % N, rand() % N, g);
}
...
void  delete_vertex_one_stapl(Graph& g){
  for(size_t i=0;i<100;++i) g.delete_vertex(i);
}
...
void  delete_vertex_two_stapl(Graph& g){
  for(size_t i=0;i<100;++i) g.delete_vertex(N-i-1);
}

template <class Graph>
void  delete_vertex_two_bgl(Graph& g){
  for(size_t i=0;i<100;++i) {
    clear_vertex(N-i-1,g);
    remove_vertex(N-i-1, g);
  }
}
...
void  add_vertices_p_stapl(Graph& g){
  for(size_t i=0;i<N;++i) g.add_vertex(typename Graph::vertex_property(i));
}

template <class Graph>
void  add_vertices_p_bgl(Graph& g){
  typedef typename graph_traits<Graph>::vertex_descriptor vertex_descriptor;
  typedef typename boost::property_map<Graph,boost::vertex_color_t>::type  vcmap_type;
  typedef typename boost::property_map<Graph,boost::vertex_name_t>::type  vpmap_type;
  
  for(size_t i=0;i<N;++i){
    vertex_descriptor vd = add_vertex(g);
    boost::get(boost::vertex_name, g)[vd]=my_vertex_property_bgl(i);
    boost::get(boost::vertex_color, g)[vd]=color_traits<boost::default_color_type>::white();
  }
}
...
void  add_edges_p_bgl(Graph& g){
  for(size_t i=0;i<EF*N;++i) 
    add_edge(rand() % N, rand() % N, weight(1,1), g);
}
...
size_t  traverse_p_bgl(Graph& g){
  size_t s=0;
  typedef typename boost::property_map<Graph,boost::vertex_name_t>::type  vpmap_type;
  typedef typename boost::property_map<Graph,boost::edge_weight_t>::type  epmap_type;
...
void  bfs_stapl(Graph& g){
  vector_property_map<graph_color<size_t> > cmap/*(g.get_num_vertices())*/;
...
void  bfs_all_stapl(Graph& g){
  ...
  typename Graph::vertex_iterator vi_end=g.end();
  for(;vi!=vi_end;++vi){
    if(cmap.get(vi)==graph_color<size_t>::white()){
...
void  bfs_internal_stapl(Graph& g){
  internal_property_map<Graph,graph_color<size_t> > cmap(g);
  visitor_test<Graph> vis;
  stapl::sequential::breadth_first_search(g,0,vis,cmap);
  bfs1 = vis.m_sum;
}
...
void  bfs_bgl(Graph& g){
  ...
  boost::breadth_first_search(g, vertex(0,g),
																														boost::color_map(make_iterator_property_map(colors.begin(), get(vertex_index,g))).visitor (vis));
  bfs2 = vis.m_sum;
}

template <class Graph>
void  bfs_all_bgl(Graph& g){
  typedef typename graph_traits<Graph>::vertex_iterator   vertex_iter;
  std::vector<default_color_type> colors(num_vertices(g),
                                         color_traits<boost::default_color_type>::white());
  visitor_bgl<Graph> vis(bfs2);
...
void  bfs_internal_bgl(Graph& g){
  visitor_bgl<Graph> vis(bfs2);
  breadth_first_search(g, vertex(0,g), boost::color_map(boost::get(boost::vertex_color, g)).
                       visitor(vis));
  bfs2 = vis.m_sum;
}
...
void  bfs_all_internal_bgl(Graph& g){
  ...
  for (vp = vertices(g); vp.first != vp.second; ++vp.first){
    if(cmap[*(vp.first)] == color_traits<boost::default_color_type>::white()){
      breadth_first_visit(g, vertex(*(vp.first),g), boost::color_map(boost::get(boost::vertex_color, g)).visitor(vis));
    }
  }
  bfs2 = vis.m_sum;
}
...
int main(int argc, char** argv){
  ...
  for(size_t it=0;it<1;++it){
    ...
    if(gvers==0)
    {
      ...
      stapl::sequential::graph<DIRECTED,MULTIEDGES> g1;
      t.start();
    ...
    if(gvers==1)
    {
      ...
      typedef stapl::graph<DIRECTED,MULTIEDGES,int,int> Graph;
      Graph bg;
      ...
      std::vector<size_t> distances(num_vertices(bg), INT_MAX);
      std::vector<size_t> indexes(num_vertices(bg), 0);
      std::vector<int> weights(num_vertices(bg));
      
      // boost::distance_map(make_iterator_property_map(distances.begin(), get(vertex_index, bg)));
      // boost::weight_map(make_iterator_property_map(indexes.begin(), get(vertex_index, bg)));
      /*
        property_map<Graph, vertex_distance_t>::type distance = get(vertex_distance, bg);
        property_map<Graph, Graph::vertex_property>::type indexmap = get(vertex_index, bg);
        property_map<Graph, Graph::edge_property>::type weightmap = get(edge_weight, bg);
      */
      prim_minimum_spanning_tree(bg, *vertices(bg).first, &p[0],
                                 make_iterator_property_map(distances.begin(), get(vertex_index, bg)),
                                 make_iterator_property_map(indexes.begin(), get(edge_weight, bg)), //weight,
                                 make_iterator_property_map(weights.begin(), get(vertex_index, bg)), //index,
    ...
    if(gvers==0)
    {
      ...
      stapl::graph<DIRECTED,MULTIEDGES, my_vertex_property, weight> g1;

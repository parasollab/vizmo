...
struct scan
{
  ...
  public:
    using in_port_size = std::integral_constant<
                           int, Compose::skeletons_size::value>;
    using in_port_type = flows::result_of::concat<
                           ...
                           stapl::tuple<
                             typename tuple_ops::result_of::back<
                               typename tuple_element<2, skeletons_t>::type
                                 ::in_port_type>::type>>;
            ...
            std::integral_constant<bool, false> /*!is_last*/)
    {
      return m_compose.template get_skeleton<0>().
               template out_port<In>(lid_offset);
    }
    ...
    {
      using in_flow_t = typename inner_ports_types<In>::in_flow_1_type;
      return flows::concat(
    ...
    {
      using in_flow_t = typename inner_ports_types<In>::in_flow_2_type;
...
struct scan_broadcast
{
  ...
  public:
    using in_port_size  = std::integral_constant<
                            int,Repeat::nested_p_type::in_port_size::value - 1>;
    ...
    in_flow(std::size_t iter_num, In const& in, std::size_t lid_offset,
            tags::repeat_iter)
    {
      using in_flow_t = typename inner_port_types<In>::in_flow_type;
      return flows::concat(
               m_repeat.template
                 nested_out_port<in_flow_t>(lid_offset, iter_num -1),
            ...
            tags::repeat_last_iter)
    {
      ...
      return flows::concat(
               m_repeat.template nested_out_port<in_flow_t>(
                                    lid_offset, iter_num - 1),
    ...
    out_port(std::size_t lid_offset)
    {
      ...
      return m_repeat.template nested_out_port<in_flow_t>(
                                 lid_offset, iter_num-1);
    }

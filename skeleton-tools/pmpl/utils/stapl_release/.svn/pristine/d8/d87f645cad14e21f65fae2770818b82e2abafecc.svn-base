...
private:
  ...
  boost::shared_ptr<set_type>                       m_set;
  bool                                              m_cont_dom;
...
public:
  ...
  /// @param cont whether or not this domain spans the entire container.
  //////////////////////////////////////////////////////////////////////
  domset1D(index_type const& lower, index_type const& upper)
    : m_set(new set_type),
      m_cont_dom(false)
  {
    m_set->add(int_type::closed(lower, upper));
  }
  ...
  domset1D(T const& lower, T const& upper, domset1D const& other)
    : m_set(new set_type),
      m_cont_dom(false)
  {
    *m_set = int_type::closed(lower, upper) & *other.m_set;
  }
  ...
  {
    return domset1D(*m_set + *other.m_set);
  }
  ...
  domset1D operator&(ODom const& other) const
  {
    ...
    if (oupper < olower)
    {
      typename ODom::index_type tmp = olower;
      olower = oupper;
      oupper = tmp;
    }
    set_type oset;
    oset.add(int_type::closed(olower, oupper));
    return domset1D(*m_set & oset);
  }
  ...
  index_type advance(index_type index, Distance n) const
  {
    if (!contains(index))

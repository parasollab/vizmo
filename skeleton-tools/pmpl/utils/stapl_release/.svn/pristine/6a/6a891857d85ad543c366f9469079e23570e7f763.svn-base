...
///
/// User-code executes under a specific context that is defined by an associated
/// @ref context object. Each @ref context has a unique @ref context_id object
/// that acts as its id.
///
...
/// @c 0 and follows the epoch of the associated @ref location_md object.
///
/// Every other context is created as the result of executing requests and they
/// have nesting level that is that of the request source + 1.
///
/// Ordering between contexts is the basis for the Causal RMI Order. Each
/// @ref context object is associated with a queue of requests which will
/// execute in a FIFO order.
///
...
// --------------------------------------------------------------------------
  ...
  aggregator_type& get_aggregator(full_location const& loc)
  {
    if (loc.get_gang_id()==get_gang_id()) {
      // Source and destination locations are on the same gang. Therefore the
      // gang metadata is the same.
      return m_aggr.emplace(
               std::piecewise_construct,
               std::forward_as_tuple(loc),
               std::forward_as_tuple(
                 *this, loc, std::ref(m_gang), is_intragang())
               ).first->second;
    }
    ...
    gang_md* const g = get_location_md().get_cached_gang_md(loc.get_gang_id());
    if (g) {
    ...
    return m_aggr.emplace(std::piecewise_construct,
                          std::forward_as_tuple(loc),
                          std::forward_as_tuple(*this, loc)).first->second;
  }
...
public:
  ...
  {
    const gang_md::id dst_gid = loc.get_gang_id();

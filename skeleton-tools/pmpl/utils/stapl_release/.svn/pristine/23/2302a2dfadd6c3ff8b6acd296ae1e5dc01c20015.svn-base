...
struct index_times_lower_dim_size<0,0>
{
  template<typename Index, typename Size>
  location_type
  operator()(Index const& id, Size const& size) const
  {
    return get<0>(id);
  }
};
...
struct index_times_lower_dim_size<Last, Last>
{
  ...
  operator()(Index const& id, Size const& size)
  {
    typedef typename tuple_element<0, Index>::type result_type;
    std::pair<result_type, result_type> lower_dim_res =
      index_times_lower_dim_size<Last, Last-1>()(id, size);
...
private:
  ...
  CID m_cid_offset;
  CID m_location_layout;
  CID m_locations_per_cid;

  void assert_layout_validity(void)
  {
#ifndef STAPL_NDEBUG
    auto t = dist_spec_impl::modulo(m_location_layout, m_num_cids);
...
public:
  ...
  uniform_ndim_to_linear_map(CID&& num_cids, CID&& loc_layout)
    : m_num_cids(num_cids),
      m_cid_offset(),
      m_location_layout(loc_layout),
      m_locations_per_cid(dist_spec_impl::divide(m_location_layout, m_num_cids))
  {

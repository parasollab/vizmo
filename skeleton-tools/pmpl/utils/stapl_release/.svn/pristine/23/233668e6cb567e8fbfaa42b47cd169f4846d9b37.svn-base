...
struct ws_neighbors
{
  ...
  {
    view.add_edge_async(v.descriptor(), (v.descriptor()+m_i)%m_n);

    if (m_bidirectional && view.is_directed()) {
      view.add_edge_async((v.descriptor()+m_i)%m_n, v.descriptor());
    }
  }
  ...
  : rand_gen
{
  ...
  void operator()(Vertex v, Graph& view)
  {
    ...
    typename Vertex::adj_edge_iterator it = v.begin(), eit = v.end();
    ...
    for (; it != eit; ++it) {
      if (this->rand(100) < m_p*100) {
        ...
        if (Delete) {
          view.delete_edge((*it).descriptor());
          if (m_bidirectional && view.is_directed()) {
            view.delete_edge(reverse((*it).descriptor()));
          }
        }
...
//////////////////////////////////////////////////////////////////////
/// @brief Generate a Watts-Strogatz small-world graph.
...
//////////////////////////////////////////////////////////////////////
/// @brief @copybrief make_watts_strogatz
/// @param n The number of nodes in the graph
/// @param k Each node is connected to k nearest neighbors in ring topology
/// @param p The probability of rewiring each edge
/// @param bidirectional True for adding back-edges, False for forward only.
...
//////////////////////////////////////////////////////////////////////
/// @brief @copybrief make_newman_watts_strogatz
/// @param n The number of nodes in the graph
/// @param k Each node is connected to k nearest neighbors in ring topology
/// @param p The probability of rewiring each edge
/// @param bidirectional True for adding back-edges, False for forward only.

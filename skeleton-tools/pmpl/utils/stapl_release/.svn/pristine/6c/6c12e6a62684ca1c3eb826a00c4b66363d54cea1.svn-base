...
class entry_renumbering
{
  ...
  static void execute_wavefront(Offsets&& offsets, Skeleton&& skeleton,
                                index_sequence<Dims...>)
  {
    skeletons::algorithm_executor<null_coarsener> exec;

    exec.execute(
      skeleton,
      offsets,
      detail::make_boundary_view<Dims>(offsets.dimensions())...
    );
  }
  ...
  static cid_type size(MDCont& md_cont)
  {
    ...
    auto mr_broadcast_skeleton =
      skeletons::compose(mr_skeleton, skeletons::broadcast_to_locs());

    // execute the skeleton to get the max cid globally
    skeletons::algorithm_executor<null_coarsener> exec;
    cid_type largest = exec.execute<cid_type>(
      skeletons::sink_value<cid_type>(mr_broadcast_skeleton), vw
    );

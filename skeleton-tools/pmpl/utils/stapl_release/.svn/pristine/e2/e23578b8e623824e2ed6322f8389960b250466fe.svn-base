...
void test(std::string alg_name, std::string filename, size_t niter,
          size_t k_start, size_t k_end, int argc, char** argv)
{
  if (algorithm_type()(alg_name) < 6)
    test_part1<Directedness>(alg_name, filename, niter, k_start, k_end, argc, argv);
  else if (algorithm_type()(alg_name) < 9)
  ...
  if (argc < 2) {
    if (stapl::get_location_id() == 0 )
      std::cerr << "usage: " << argv[0] << " [--filename input-filename] "
        << " [--algorithm "
        << "{bfs, cc, pagerank, sssp, kcore, mssp, topsort, scc, coloring}]"

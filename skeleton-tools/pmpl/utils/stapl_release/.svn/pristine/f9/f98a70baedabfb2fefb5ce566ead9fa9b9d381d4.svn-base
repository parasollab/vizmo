...
{
  gang_md* const g = gang_md_registry::try_get(init_gid);
  if (!g)
...
void context::flush_fence_md(void)
{
  ...
  const gang_md::id init_gid = get_initiator().get_gang_id();
  const gang_md::id gid      = get_gang_id();
  const nesting_level n      = get_nesting();

  gang_md::fence_md* f = nullptr;
  if (init_gid==gid) {
    // intergang context, but from the same initiator gang
    STAPL_RUNTIME_ASSERT(!m_intergang_md);
    f = &(m_gang.get_fence_md());
  }
    ...
    // intergang context
    gang_md* const g       = gang_md_registry::try_get(init_gid);
    const process_id owner = gang_md_registry::id_owner(init_gid);
    ...
    if (!g) {
      ...
      if (m_proc==0) {
        rpc<
          decltype(&fence_md_update_sent), &fence_md_update_sent
        >(owner, init_gid, gid, n, m_sent);
        m_sent = 0;
      }
      else if (m_sent==0) {
        rpc<
          decltype(&fence_md_update_processed), &fence_md_update_processed
        >(owner, init_gid, gid, n, m_proc);
        m_proc = 0;
      }
      else {
        rpc<
          decltype(&fence_md_update), &fence_md_update
        >(owner, init_gid, gid, n, m_sent, m_proc);
        m_sent = 0;
        m_proc = 0;
      }
      return;
                                      ...
                                      const unsigned int N)
{
  ...
  if (init_gid==get_gang_id()) {
    m_gang.get_fence_md().update_sent(gid, n, N);
  }
  else {
    if (!m_intergang_md)

...
public:
  ...
  typedef typename scheduler_type::sched_info_type sched_info_type;
  typedef stapl::executor_base::result_type        result_type;
...
private:
  typedef typename scheduler_type::entry_type      sched_entry_type;
  ...
  public:
    ...
    {
      const result_type r = static_cast<task_type&>(*m_p)();
...
protected:
  ...
  {
    entry_type* const entry = new entry_type(t, sched_info);
  ...
  {
    entry_type* const entry = new entry_type(t, std::move(sched_info));
    m_scheduler.add_active(*entry);
    this->notify_runnable();
  }
  ...
  {
    entry_type* const entry = new entry_type(t, stapl::none);
    m_scheduler.add_active(*entry);
    this->notify_runnable();
  }
  ...
  result_type do_progress(void)
  {
    ...
    entry_type& entry = static_cast<entry_type&>(m_scheduler.next());
    entry();
    return status();
  }

...
public:
  ...
  {
    typename Vw::iterator vi = v.begin(), vi_end = v.end();
...
struct parallel_reader
{
                  ...
                  size_t blk_sz = 4096, bool sharded = false)
  {
    ...
    {
      add_edge_aggregator<typename Graph::edge_descriptor, Graph> edge_aggr(g);
                                       ...
                                       LineReader line_reader)
{
  ...
  size_t loc_id = get_location_id();
  size_t shards_per_loc = num_shards/get_num_locations();
  if (shards_per_loc == 0 && loc_id < num_shards)
    shards_per_loc = 1;

  size_t shards_start = shards_per_loc * loc_id;
  size_t shards_end = shards_start + shards_per_loc;

  // The last location gets any remaining shards.
  if (loc_id == get_num_locations() - 1)
    shards_end += (num_shards % get_num_locations());

  for (size_t i = shards_start; i < shards_end; ++i) {
    std::ifstream ifile;

...
public:
  ...
  nested_out_port(std::size_t repeat_lid_offset, std::size_t iter_num) const
  {
    return nested_skeleton().template
            out_port<In>(lid_offset(repeat_lid_offset, iter_num));
  }
...
public:
  ...
  {
    m_skeleton.S::set_dimensions(spawner, view_domains);

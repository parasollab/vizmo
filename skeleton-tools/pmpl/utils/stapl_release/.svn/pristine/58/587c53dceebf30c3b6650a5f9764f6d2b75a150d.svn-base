...
//#define USE_INLINE_FLOWS

#ifndef USE_INLINE_FLOWS
#include "flows/ft_layout_0d.hpp"
#include "flows/ft_layout_1d_fwd.hpp"
#include "flows/ft_layout_1d_rev.hpp"
#include "flows/transpose_3d.hpp"
#endif
...
struct fft
{
  ...
  void operator()(X1&& x1, X2&& x2)
  {
    ...
    if (m_config.layout == layout_0d) {
      ...
      #ifndef USE_INLINE_FLOWS
        auto ft_skeleton =
          compose< ::flows::ft_layout_0d>(
            zip<2>(make_cffts<false>(
                     cffts1<val_t>(1, m_config.dims[0], info, m_config))),
            zip<3>(make_cffts<true>(
                     cffts2<val_t>(1, m_config.dims[1], info, m_config))),
            zip<3>(make_cffts<true>(
                     cffts3<val_t>(1, m_config.dims[2], info, m_config)))
          );
      #else
        auto ft_skeleton =
     ...
     //if it is a xy_z transformation
      ...
      auto transpose_skeleton =
        compose< ::flows::transpose_3d>(
          skeletons::zip<3>(make_cffts<true>(
                              prepare_transpose<val_t>(d0, d1, m_config.np2))),
          alltoall<alltoall_data_t, skeletons::tags::pairwise_exchange>(),
          skeletons::zip<3>(transpose_finish(d0, d1)));
      #else
      auto transpose_skeleton =
        compose<tags::inline_flow>(
          ...
          (v2 = skeletons::zip<3>(transpose_finish(d0, d1)))[v1, input1, input2]
        );
      #endif
      ...
      if (m_is_forward) {
        #ifndef USE_INLINE_FLOWS
        auto ft_skeleton =
          compose< ::flows::ft_layout_1d_fwd>(
            zip<2>(make_cffts<false>(
                     cffts1<val_t>(1, m_config.dims[0], info, m_config))),
            zip<3>(make_cffts<true>(
                     cffts2<val_t>(1, m_config.dims[1], info, m_config))),
            transpose_skeleton,
            zip<3>(make_cffts<true>(
                     cffts1<val_t>(1, m_config.dims[2], info, m_config)))
          );
        #else
        auto ft_skeleton =
          compose<tags::inline_flow>(
            ...
            (v3=zip<3>(make_cffts<true>(cffts1<val_t>(1, m_config.dims[2], info,
                                        m_config))))[v2, input1, input1]
          );
        #endif
        algorithm_executor<>().execute(
          ft_skeleton,
        ...
        #ifndef USE_INLINE_FLOWS
        auto ft_skeleton =
          compose< ::flows::ft_layout_1d_rev>(
            zip<2>(make_cffts<false>(
                     cffts1<val_t>(-1, m_config.dims[2], info, m_config))),
            transpose_skeleton,
            zip<3>(make_cffts<true>(
                     cffts1<val_t>(-1, m_config.dims[1], info, m_config))),
            zip<3>(make_cffts<true>(
                     cffts2<val_t>(-1, m_config.dims[0], info, m_config)))
          );
        #else
        auto ft_skeleton =
          compose<tags::inline_flow>(
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  typedef stapl::array<value_t>                     complex_array_t;
  typedef stapl::array<inner_value_t>               double_array_t;
  typedef stapl::array<fft_timers>                  timers_array_t;
  ...
  local_timer.start();
  exec.execute(indexmap_s, twiddle_factors);
  exec.execute(initial_condition_s, native_view(u1));
  ...
  for (int_t i = 1; i <= traits.niters; ++i) {
    local_timer.reset();
    local_timer.start();
    exec.execute(evolve_s, u0, u1, twiddle_factors);

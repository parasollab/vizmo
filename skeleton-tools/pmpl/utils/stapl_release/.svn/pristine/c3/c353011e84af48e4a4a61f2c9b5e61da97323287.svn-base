...
public:
  ...
  : m_status(ACTIVE)
  {
    ...
    if (!ctx) {
      ...
      location_md const& l       = ctx->get_location_md();
      const location_md::id nlid = mf(l.get_id());
      ...
      location_md& nl = create(g.get_id(),
                               rf(0),
  ...
  : m_status(SWITCHED)
  {
    ...
    context* const ctx = this_context::try_get();
    location_md& l     = const_cast<T&>(t).get_location_md();

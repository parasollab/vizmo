...
{
  typedef T result_type;
  skeletons::wavefront_utils::wavefront_filter_direction m_direction;

  void set_direction(
    skeletons::wavefront_utils::wavefront_filter_direction direction)
  {
  ...
  bool operator==(no_op_filter const& other) const
  {
...
struct sum_op<3, T>
{
  ...
  /// @param ws the rest of the inputs from the wavefront
  ///
...
struct sum_op<2, T>
{
  ...
  void set_directions(
         std::array<skeletons::wavefront_utils::wavefront_filter_direction, 2>)
  { }
                    ...
                    std::array<skeletons::position, 3> corners)
{
  ...
  auto wavefront_skeleton = wavefront<2>(
                              sum_op<3, value_type>(), corners,
                              no_op_filter<value_type>());
  ...
  STAPL_TEST_REPORT((view1[l] == expected_sum) and
                    (view2[l] == expected_sum), title);
                    ...
                    std::array<skeletons::position, 2> corners)
{
  ...
  auto wavefront_skeleton = wavefront(sum_op<2, value_type>(), corners);

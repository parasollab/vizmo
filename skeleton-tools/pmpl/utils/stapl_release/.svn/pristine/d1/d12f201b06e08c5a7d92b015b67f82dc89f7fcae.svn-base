...
public:
  ...
  element_ref_cache(Container*, Index const&, bool)
  { }
...
public:
  ...
  /// @param is_local boolean indicating whether the container element
  ///        is stored locally in the outer container.
  ///
  ...
  //////////////////////////////////////////////////////////////////////
  template<typename Container, typename Index>
  element_ref_cache(Container* ct_ptr, Index const& index, bool is_local)
...
public:
  ...
  container_accessor(container_type* container, index_type const& index)
    : base_type(container, index, container->is_local(index)),
  ...
  value_type read() const
  {
    ...
    // else
    return sync_rmi(0, container().get_rmi_handle(),
                    &Container::get_element, m_index);
  }
  ...
  void write(const T& val) const
  {
    ...
    if (container_pointer_valid())
    {
      container().set_element(m_index, val);
      return;
    }

    //else
    async_rmi(0, container().get_rmi_handle(),
              &Container::set_element, m_index, val);
  }
  ...
  void apply_set(const F& f) const
  {
    ...
    async_rmi(0, container().get_rmi_handle(), mem_fun, m_index, f);
  }
  ...
  apply_get(F const& f) const
  {
    ...
    async_rmi(0, container().get_rmi_handle(),
              mem_fun, m_index, wrapper_type{f, std::move(p)});

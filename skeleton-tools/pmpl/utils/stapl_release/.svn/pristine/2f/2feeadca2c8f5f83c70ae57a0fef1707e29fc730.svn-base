...
struct ex_305b_build_wf {
  typedef void result_type;
  template <typename SegRef, typename MapRef>
  result_type operator()(SegRef seg_key, SegRef seg_fac, MapRef map_ary_vw) {
    typename SegRef::value_type key = *seg_key.begin(); // ## 8

    map_ary_vw[key].resize(seg_fac.size());

    size_t j = 0;
    typename SegRef::iterator iter_fac = seg_fac.begin(); // ## 9

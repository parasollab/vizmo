...
public:
  ...
  void spawn_element(std::size_t tid, WF&& wf, std::size_t no_succs,
                     In&&... in) const
  {
    spawn_element<isResult>(tid, std::vector<std::size_t>(),
                     ...
                     In&&... in) const
  {
    ...
    std::cout << "Task(" << tid << ") with " << no_succs << " successors "
              << " depends on [";
  ...
  {
    std::cout << "Task(" << tid << ") successors changed to " << no_succs
      << std::endl;
  }
...
private:
  ...
  void
  print_dependency(flows::view_producer_info<View> const& producer) const
  {
  ...
  void
  print_dependency(
    flows::reflexive_producer_info<Element, isRef> const& producer) const
  {
    skeletons::show(producer.get_element(), std::cout);
  }
  ...
  template <typename T>
  void
  print_dependency(flows::constant_producer_info<T> const& producer) const
  {

...
//////////////////////////////////////////////////////////////////////
/// @brief Specialization of package_container_storage for the
/// @ref counting_container, which is the container underlying a
/// @ref counting_view.
///
...
/// the counting_container itself isn't a p_object, but one of its data
/// members is currently a p_object.
//////////////////////////////////////////////////////////////////////
template<typename T>
class package_container_storage<view_impl::counting_container<T>, false, false>
{
private:
  view_impl::counting_container<T>* m_ct;

public:
  package_container_storage(view_impl::counting_container<T>* const ct_ptr)
    : m_ct(ct_ptr)
  { }

  view_impl::counting_container<T>* container_ptr(void) const
  {
    return m_ct;
  }
...
public:
  ...
  {
    // If the container is a view itself, pass the pointer to the constructor so
    // the view being returned takes ownership of the new object being created
    // in the package_container_storage specialization for views.
    if (is_view<container_t>::value)
...
public:
  ...
  {
    return result_type(m_ct_ptr.container_ptr(), m_dom, m_mf, m_id, m_md);
  }
...
public:
  typedef repeat_view<Container> result_type;
  ...
  result_type operator()(void) const
  {
    value_type* value_ptr = m_value.container_ptr();
    result_type res(*value_ptr);
    delete value_ptr;
    return res;
  }
...
//////////////////////////////////////////////////////////////////////
/// @brief Support class of @ref transporter_unpackager polymorphic functor. If
///   the specified argument is a @ref package, invoke the function operator.

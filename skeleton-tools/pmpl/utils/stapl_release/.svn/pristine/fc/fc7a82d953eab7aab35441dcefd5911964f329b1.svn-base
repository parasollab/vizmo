  ...
  : p_object
{
  ...
  {
    stapl_assert(0, "Why are you packing repeat_view?");
  }
  ...
  {
    stapl_assert(0, "repeat_view::get_num_copies called");
  ...
  {
    stapl_assert(0,
      "Why are you calling detail::repeat_view::get_num_subviews()?");
...
public:
  ...
  void define_type(typer&)
  {
    stapl_assert(0, "map_subview define_type called...");
  }
}; // struct map_subview
...
public:
  ...
  void define_type(typer&)
  {
    stapl_assert(0, "Why are you packing map_view?");
  }
  ...
  operator[](std::size_t idx) const
  {
    ...
    // stapl_assert(0, "Once again map_view::operator[] needs updating");

    //stapl_assert(get_location_id() < get_num_subviews(),
    //"map_view::operator[] called on loc with no elements (doesn't know size");

    // std::size_t block_size = this->subview().size();
    // std::size_t home_loc   = idx / block_size;
    // std::size_t local_idx  = idx % block_size;

    // if (home_loc == get_location_id())
    //   return this->subview()[local_idx];

    // typedef value_type (edge_container<T>::* mem_fun_t)
    //   (std::size_t, get_element_func<T>, df_request_type) const;

    // mem_fun_t mem_fun
    //   = &edge_container<T>::template apply_get<get_element_func<T> >;

    // return sync_rmi(
    //  home_loc,
    //  proxy_core_access::accessor(m_ref).m_view.container().get_rmi_handle(),
    //  mem_fun,
    //  result,
    //  get_element_func<T>(local_idx),
    //  df_request_type(FULL));
  }
  ...
  get_local_component(size_t idx) const
  {
    ...
    /// const bool cond = m_n_copies != 1 || loc < m_n_subviews;
    ///
    /// if (!cond)
    ///   std::cout << get_location_id() << ": MV_GLC - " << idx << ", "
    ///             << m_n_copies << ", " << loc << "\n";
  ...
  subview_type get_subview(cid_type idx) const
  {
    ...
    // return m_base_view.get_element(idx);
...
#   undef STAPL_SKELETONS_MAP_RESULT2
  ...
  else
  {
    ...
    for (; !iter0.at_end();)
    {
       // stapl_assert(*iter0 == stapl::get_location_id(),
       //   "composition::map_factory, only balanced distributions for now");
       // std::cout << get_location_id() << ": map_factory::add_task() "
       //           << *iter0 << "\n";

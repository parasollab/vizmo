...
private:
  ...
  /// Unused parameter is type tag dispatch from @p operator().
  //////////////////////////////////////////////////////////////////////
  void release(std::false_type, tg_callback const& cb)
  {
    vs_map(lazy_ref_release_func(cb), this->viewset().views());
  }
...
public:
  ...
  ~Task() final
  {
    if (EnablePersistence::value)
      vs_map(lazy_ref_release_persistent_func(), this->viewset().views());
  }
};
...
public:
  ...
  identity_task_impl(detail::edge_entry_base* edge_entry_ptr,
                     SchedInfoParam&& si, WFParam const&,
                     tg_callback const& cb, ViewParam&& view)
    : task_base_intermediate_t(
        edge_entry_ptr, std::forward<SchedInfoParam>(si)),
      ViewSet(cb, std::forward<ViewParam>(view))
  { }
  ...
  result_type operator()(tg_callback const& cb) final
  {
    ...
    if (in_edge.is_direct_storage())
    {
      if (in_edge.stealable())
        this->processed(in_edge.steal(), b_has_succs, cb);
      else
        this->processed(in_edge.get_storage_ref(), b_has_succs, cb);
    }
    else
      this->processed(in_edge.get_shared_wrapper(), b_has_succs, cb);

    post_execute_func()(in_edge);

    if (EnablePersistence::value == false)
    {
      lazy_ref_release_func f(cb);
      f(in_edge);
    }
  ...
  ~identity_task_impl() override
  {
    if (EnablePersistence::value)
    {
      lazy_ref_release_persistent_func f;
      f(get<0>(static_cast<ViewSet&>(*this).views()));
    }
  }
...
{
private:
  typedef typename task_base:: result_type result_type;
...
public:
  factory_task(std::size_t /*tid*/, Factory factory, ViewSet& vs)
    : m_factory(std::move(factory)), m_vs(vs)
  { }

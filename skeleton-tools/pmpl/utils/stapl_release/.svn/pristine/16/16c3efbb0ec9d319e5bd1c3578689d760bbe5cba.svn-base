...
public:
  ...
  simple_object(simple_object const&) = delete;
  simple_object& operator=(simple_object const&) = delete;
...
{
private:
  rmi_handle::reference m_ref_handle;
  unsigned int                 m_id;
  unsigned int                 m_lt, m_rt; // neighbor id's

public:
  p_test(void)
  : m_ref_handle(get_rmi_handle()),
  ...
  void test_in_order(void)
  {
    ...
    std::vector<simple_object*> v;
    for (int i=0; i<N; ++i) {
      v.push_back(new simple_object(i));
    }
  ...
  void test_rmi_handle_packing(void)
  {
    ...
    async_rmi(m_lt, m_ref_handle, &p_test::callback, ref, m_id);
    async_rmi(m_lt, m_ref_handle, &p_test::callback_const, cref, m_id);

    rmi_fence(); // wait for async_rmi calls to finish before checking
    STAPL_RUNTIME_TEST_CHECK(o.get(), int(m_id));
  ...
  void test_conversions(void)
  {
    ...
    // test conversions - commented out ones are not allowed
    {

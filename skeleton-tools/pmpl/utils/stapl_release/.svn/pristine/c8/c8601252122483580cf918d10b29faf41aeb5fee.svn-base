...
//////////////////////////////////////////////////////////////////////
/// @brief Registry for SPMD distributed objects.
///
...
///
/// It is assumed that all objects are registered in the same order on all
/// locations of the gang, so that the handles will be assigned in the same
/// order in each location.
///
...
{
public:
  typedef object_id                handle_type;
  typedef logical_clock::time_type epoch_type;
  typedef std::size_t              size_type;
private:
  typedef std::vector<handle_type> urb_type;

  static_assert(sizeof(size_type)>=sizeof(handle_type), "Cannot store handle");
  ...
  {
    for (size_type i = index; i < (m_capacity-1); ++i) {
...
public:
  ...
  //////////////////////////////////////////////////////////////////////
  /// @brief Registers an object and returns a handle to it.
  //////////////////////////////////////////////////////////////////////
  template<typename T>
  handle_type object_register(T* const p, const epoch_type e)
  {
    if (m_index==0) {
      if (!m_registry) {
  ...
  //////////////////////////////////////////////////////////////////////
  void object_unregister(const handle_type h, const bool immediate = false)
  {
  ...
  //////////////////////////////////////////////////////////////////////
  void* object_get(const handle_type h) const noexcept
  {
    STAPL_RUNTIME_ASSERT_MSG(h!=invalid_object_id, "Invalid handle.");
    if (h>=m_capacity)
      return nullptr;
    data_type& t = m_registry[h];
    return (t.valid() ? t.get_data() : nullptr);
  }
  ...
  //////////////////////////////////////////////////////////////////////
  void* object_get(const handle_type h, const epoch_type e) const noexcept
  {
    STAPL_RUNTIME_ASSERT_MSG(h!=invalid_object_id, "Invalid handle.");
    STAPL_RUNTIME_ASSERT_MSG(e!=logical_clock::no_time, "Invalid epoch.");
    if (h>=m_capacity)
      return nullptr;
    data_type& t = m_registry[h];
    return ((t.valid() && (t.get_epoch()==e)) ? t.get_data() : nullptr);
  }
};

...
public:
  ...
  ///                 evaluated in
  /// @param view     the set of input/output data passed to the skeleton
  //////////////////////////////////////////////////////////////////////
  ...
  void execute(Skeleton&& skeleton, Env env, V&... view)
  {
    ...
    skeleton.set_dimensions(
               spnr,
               make_tuple(
                 skeletons::domain_type<
                   typename V::domain_type, has_finite_domain<V>::value
                 >(view.domain())...
               ));
    spnr.spawn(std::forward<Skeleton>(skeleton), 0,
               stapl::tuple<>(), stapl::tuple<>(),
               stapl::make_tuple(flows::make_view_flow(view)...),

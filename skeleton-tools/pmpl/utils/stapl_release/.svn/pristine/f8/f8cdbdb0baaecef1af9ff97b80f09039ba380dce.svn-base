...
template <typename Spawner, typename VSizes>
struct set_dimensions_helper
{
  Spawner const& m_spawner;
  VSizes const& m_view_sizes;

  set_dimensions_helper(Spawner const& spawner, VSizes const& view_sizes)
    : m_spawner(spawner),
      m_view_sizes(view_sizes)
  { }

  template <typename S>
  void operator()(S& skeleton) const
  {
    skeleton.set_dimensions(m_spawner, m_view_sizes);
  }
};
...
public:
  ...
  using in_port_size      = typename ports_t::in_port_size;
...
public:
  ...
  compose(Skeletons const& skeletons)
  ...
  void set_dimensions(Spawner const& spawner, VDomains const& view_domains)
  {
    tuple_ops::for_each(m_skeletons,
                        skeletons_impl::set_dimensions_helper<
                          Spawner, VDomains
                        >(spawner, view_domains));
  }
private:
...
public:
  ...
  out_port(std::size_t id_offset) const
  {
    return ports_t(*this).template out_port<In>(id_offset);
  }
  ...
  get_out_port(std::size_t id_offset) const
  {
    return this->template get_skeleton<i>().template out_port<In>(
             this->template lid_offset<i>(id_offset));
  }

...
struct piped
{
  ...
  public:
    using in_port_size   = typename tuple_element<
                             0, skeletons_t>::type::in_port_size;
    using in_port_type   = typename tuple_element<
                             0, skeletons_t>::type::in_port_type;
    ...
    >::type::out_port_type
    in_flow(In const&, std::size_t lid_offset,
            std::integral_constant<int, i>,
            std::integral_constant<bool, B> /*is_last*/)
    {
      using in_flow_t = typename inner_ports_types<In, i-1>::in_flow_type;

      return m_compose.template get_skeleton<i-1>().
               template out_port<in_flow_t>(
    ...
    out_port(std::size_t lid_offset)
    {
      ...
      using inner_flow_t = typename inner_ports_types<
                             In, skeletons_size::value - 1>::in_flow_type;

      return m_compose.template get_skeleton<
               skeletons_size::value - 1
             >().template out_port<inner_flow_t>(lid_offset);
    }
  };
...
struct input_to_all
{
  ...
  protected:
    Compose const& m_compose;
  public:
    using in_port_size   = std::integral_constant<int, skeletons_size::value>;
    ...
    in_flow(In const& in, std::size_t lid_offset,
            ...
            std::integral_constant<bool, B> /*is_last*/)
    {
      using inner_flow_t = typename inner_ports_types<In, i-1>::in_flow_type;
      return flows::concat(
               m_compose.template get_skeleton<i-1>().
                 template out_port<inner_flow_t>(
                  m_compose.template lid_offset<i-1>(lid_offset)),
               in
            );
    }
...
{
  using in_flow_type = result_of::concat<
                         typename inner_ports_types<Compose, In, i-1>
                           ::out_port_type,

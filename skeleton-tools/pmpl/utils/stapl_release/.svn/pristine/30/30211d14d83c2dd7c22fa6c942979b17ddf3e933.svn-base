...
//////////////////////////////////////////////////////////////////////
/// @file
/// Benchmark for @ref stapl::async_rmi() / @ref stapl::try_rmi() latency.
///
                  ...
                  distributed_object& d_obj, const std::size_t size)
{
  ...
  if (deferred_sync) {
    for (int i = 0; i < skip + loop; i++) {
      ...
      async_rmi(1, r, &distributed_object::put_ack<decltype(w)>, w);
    }
    d_obj.wait_done();
  }
  else {
    for (int i = 0; i < skip + loop; i++) {
      ...
      async_rmi(1, r, &distributed_object::put<decltype(w)>, w);
    }
                  ...
                  distributed_object& d_obj, const std::size_t size)
{
  ...
  for (int i = 0; i < skip + loop; i++) {
    ...
    async_rmi(1, r, &distributed_object::put_ping<decltype(w)>, w);
    d_obj.wait_done();
  }
  const double t = counter.stop();

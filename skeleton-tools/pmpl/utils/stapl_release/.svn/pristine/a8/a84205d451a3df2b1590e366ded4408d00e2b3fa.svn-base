...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  typedef stapl::array<val_t>       container_t;
  ...
  typedef stapl::array<std::vector<val_t>> ranks_container_t;
  ...
  auto is_skeleton =
  #ifndef USE_INLINE_FLOWS
         skeletons::compose< ::flows::nas_is>(
           map(bucket_cardinality(traits.num_buckets(), shift)),
           allreduce<tags::right_aligned>(vec_plus_t()),
           map(bucket_and_element_offset(keys_array_size, n_locs)),
           zip<3>(prepare_for_alltoall<alltoall_val_t>(traits.num_buckets(),
                                                       shift)),
  ...
  p();
#else
  algorithm_executor<coarsen_all_but_last<default_coarsener>> exec;
  exec.execute(is_skeleton, keys_view, make_repeat_view(ranks_view));
#endif
  ...
  stapl::map_func(copy_to_sorted_view(),
                  ranks_view, make_repeat_view(sorted_key_view));

  bool correctly_sorted = stapl::is_sorted(sorted_key_view);

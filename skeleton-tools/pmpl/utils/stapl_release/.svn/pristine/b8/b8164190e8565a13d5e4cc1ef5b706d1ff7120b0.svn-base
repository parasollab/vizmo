...
//#define USE_INLINE_FLOW

#include "stapl_wf.h"
#ifndef USE_INLINE_FLOW
#include "flows/fmm.hpp"
#endif
#include <type_traits>
...
void main_work(Args args)
{
...
#ifndef USE_INLINE_FLOW
  auto bounds_skeleton = skeletons::compose<flows::fmm>
  (
    skeletons::map(generateBodies),
    skeletons::zip_reduce<1>(getLocalBounds, allReduceBounds),
    skeletons::broadcast_to_locs(),
    skeletons::zip<3>(calculateBucketCadinality),
    skeletons::alltoall<Bodies, AllToAllTag>(),
    skeletons::map(flattenBodies),
    skeletons::map(getNewLocalBounds),
    skeletons::zip<2>(buildTree),
    skeletons::zip<2>(getCellBounds),
    skeletons::zip<2>(updwardPass),
    skeletons::allgather<Bounds,AllGatherTag>(),
    skeletons::zip<2>(setBounds),
    skeletons::zip<3>(setLET),
    skeletons::zip<2>(prepareSendBodies),
    skeletons::alltoall<Bodies, AllToAllTag>(),
    skeletons::zip<2>(prepareSendCells),
    skeletons::alltoall<Cells, AllToAllTag>(),
    skeletons::map(dualTreeTraversal),
    skeletons::zip<5>(getLET_Traverse),
    skeletons::map(downwardPass),
    skeletons::zip<3>(keepAlive)
  );
#else
  ...
  auto bounds_skeleton = skeletons::compose<skeletons::tags::inline_flow>
  (
    ...
    (v8 = skeletons::zip<2>(getCellBounds))[v7, v6],
    (v9 = skeletons::zip<2>(updwardPass))[v7, v5],
    (v10= skeletons::allgather<Bounds,AllGatherTag>())[v8],
    (v11= skeletons::zip<2>(setBounds))[tree_stapl_in, v10],
    (v12= skeletons::zip<3>(setLET))[v11, v9, tree_stapl_in],
    ...
    (v17= skeletons::map(dualTreeTraversal))[v9],
  ...
  skeletons::algorithm_executor<coarsen_all_but_last<default_coarsener>> exec;
  exec.execute(bounds_skeleton, bodies_vw, tree_stapl_vw);

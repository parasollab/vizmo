...
public:
  spawner(Env env, memento& memento_stack)
    : m_env(env),
    ...
    switch (m_spawn_mode) {
      case DEFAULT:
        this->m_env.template spawn_element<isResult>(
          tid, std::forward<WF>(wf), no_succs,
          std::forward<In>(in)...);
        break;
      case SET_HOLD:
        this->m_env.template spawn_element<isResult>(
          tid, std::forward<WF>(wf), stapl::defer_spec,
          std::forward<In>(in)...);
        break;
      case SET_NUM_SUCCS:
                     ...
                     In&&... in)
  {
    switch (m_spawn_mode) {
      case DEFAULT:
        this->m_env.template spawn_element<isResult>(
          tid, notifications, std::forward<WF>(wf), no_succs,
          std::forward<In>(in)...);
        break;

...
struct container_metadata_projection
{
  ...
  return_type* operator()(View* vw, P* part) const
  {
  ...
  return_type* project_based_on_dimensionality(View* vw, P* part,
    ...
    std::integral_constant<bool, true>) const
  {
    ...
    return_type* out_part = new return_type();
    // Local elements loop
    ...
    for (size_t i = 0; i != nsubdomains; ++i)
    {
      ...
      if (!subdomain.empty()) {
        ...
        typename domain_type::index_type first_e = subdomain.first();
        typename domain_type::index_type prev_e  = subdomain.first();
        typename domain_type::index_type curr_e  = subdomain.first();
               ...
               prev_e = subdomain.advance(prev_e, 1), ++li)
          {
                ...
                (e_info.id() != curr_info.id()))
            {
              out_part->push_back_here(dom_info_type(
                ...
                coarsen_partition_impl::construct_domain<domain_type>()(

...
: m_impl(new shared_runqueue_impl)
{ }
...
: m_impl(new shared_runqueue_impl{first, n})
...
: m_impl(new shared_runqueue_impl{location_range(r)})
{ }
...
: m_impl(new shared_runqueue_impl{*other.m_impl})
{ }
...
{
  if (gd.is_on_shmem()) {
    m_impl.reset(new shared_runqueue_impl{0, gd.get_num_locations()});
  }
  else if (gd.is_on_distmem()) {
    m_impl.reset(new shared_runqueue_impl);
  }
  else {
    m_impl.reset(
      new shared_runqueue_impl{location_range(gd.get_location_ids(pid))});
  }
}
...
// Acquires an already created shared_runqueue_impl or creates a new one
...
static void runqueue_add(message_ptr m)
{
  switch (m->type()) {
    ...
    case header::FWD_LOCATION_RPC: {
      ...
      auto const& h  = m->deduce_payload_offset<header::location_rpc>();
    ...
    case header::BCAST_LOCATION_RPC: {
      // attempt to queue, if the gang metadata is not present, defer
      auto const& h =
        m->deduce_payload_offset<header::bcast_location_rpc>();
    ...
    case header::FWD_RMI: {
      // gang metadata is here, forward it to the correct destination if needed
      auto const& h      = m->deduce_payload_offset<header::request>();
    ...
    case header::FWD_BCAST_RMI: {
      ...
      auto const& h  = m->deduce_payload_offset<header::bcast_request>();
    ...
    case header::COLL_GATHER: {
      auto const& h = m->deduce_payload_offset<collective::id>();
      collective::notify_arrival(h, std::move(m));
    } break;
...
void runqueue::forward(const gang_id dst_gid, message_ptr m)
{
  ...
  const process_id owner = gang_md_registry::id_owner(dst_gid);
...
{
  return std::alignment_of<runqueue_impl>::value;
}


// Constructs a new runqueue
runqueue::runqueue(void* p, location_md& l)
: m_impl(runqueue_impl::create(p, l))
{ }

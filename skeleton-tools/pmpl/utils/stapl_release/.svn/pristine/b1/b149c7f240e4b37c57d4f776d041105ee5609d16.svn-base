...
void test_1d_1d_3d(const std::size_t n, const std::size_t m)
{
  ...
  STAPL_TEST_REPORT(passed, "Composed slices + skeletons (1-1-3)");
}
...
void test_3d_1d_1d(const std::size_t n, const std::size_t m)
{
  ...
  typedef skeletons::spans::balanced<3>  span_t;

  skeletons::algorithm_executor<multiview_coarsener<false>>().execute(
    skeletons::coarse(skeletons::zip<2, span_t>(set_is_local())), sv2, sv2
  );

  auto linear = linear_view(v);
  const bool passed = stapl::count(linear, true) == v.size();

  STAPL_TEST_REPORT(passed, "Composed slices + skeletons (3-1-1)");
}

...
private:
  typedef typename boost::mpl::if_c<
    b_coarse,
    MapWF,
    coarse_map_reduce_wf<MapWF, ReduceWF>
  >::type                                              coarse_mr_t;
...
struct
map_reduce_factory<MapWF, ReduceWF, b_coarse>::
result<map_reduce_factory<MapWF, ReduceWF, b_coarse>
  (BOOST_PP_ENUM_PARAMS(i, V))>
  : public boost::result_of<
      typename map_reduce_factory<MapWF, ReduceWF, b_coarse>::coarse_mr_t
        (BOOST_PP_ENUM(i, STAPL_SKELETONS_MR_RESULT, ~))
    >
{ };
...
template<typename Map, typename Reduce, BOOST_PP_ENUM_PARAMS(i, typename V)>
struct map_reduce<Map, Reduce, BOOST_PP_ENUM_PARAMS(i, V)>
  : public boost::result_of<
      paragraph<default_scheduler,
             map_reduce_factory<Map, Reduce>,
             BOOST_PP_ENUM_PARAMS(i,V)
            >()
    >
{ };
...
template<typename Map, typename Reduce, BOOST_PP_ENUM_PARAMS(i, typename V)>
inline
typename result_of::map_reduce<Map, Reduce, BOOST_PP_ENUM_PARAMS(i, V)>::type
map_reduce(Map const& fmap, Reduce const& freduce,
           BOOST_PP_ENUM_BINARY_PARAMS(i, V, const& v))
{
  typedef map_reduce_factory<Map, Reduce>                 factory_t;

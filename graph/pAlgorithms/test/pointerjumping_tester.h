/* Tester class for parallel  PointerJumping technique.
 * to test:
 * mpirun -np ? PointerJumping_Testers 10(aggregation) -testall #elets
...
#include "pAlgorithms/PointerJumping.h"
#include <cmath>
...
/* Tester_PointerJumping : Tester for Pointer Jumping technique
 * the testing case is computing prefix sums for a pList<double>
 * whose each subPContainer has only 1 integer.
 */
class Tester_PointerJumping : public BaseTester {
private:
  int n_elet; 
  int each, rem;
...
public:
  Tester_PointerJumping() {flag='p'; n_elet=0;}

  char* GetName() { return "Tester_PointerJumping"; }
  
  BaseTester *CreateCopy() { Tester_PointerJumping *t = new Tester_PointerJumping; t->flag=this->flag;  t->n_elet=this->n_elet;  return t; }
  ...
  void Test() {
    ...
    MethodNames.push_back("PointerJumping");
    ...
    //to test the new method added into BasePContainer: CopyStructure
    //we build another pContainer pBList<double> y which has exactly the same
    //boundary info. as x
    pList<double> y;
    stapl::rmi_fence();
    y.CopyStructure<pList<int> >(x);
    stapl::rmi_fence();
    for (PARTID partid=0; partid<y.get_num_parts(); partid++) {
      y.AddElement2Part( myid+partid*nprocs, partid);
    }
    stapl::rmi_fence();
    //y.DisplayPContainer();
    //y.GetDistribution().DisplayBoundaryInfo();
    //stapl::rmi_fence();

    /* Pointer jumping is only a technique, not a complete algo.,
    ...
    TODOFunc<double> todofunc(Forward);
    PointerJumping<pList<double>,TODOFunc<double> >(y, todofunc);
    stapl::rmi_fence();
    ...
    if_want_corr { //only check the last sum, if it's not the total sum, wrong
      int npart = y.get_num_parts();
      if ( (rem>0 && myid==rem-1 && partid==npart-1) || (rem==0 && myid==nprocs-1 && partid==npart-1)) { //last part, should have totalsum
        double tmp = (*( (*(y.GetPart(partid)) ).part_data.begin() ) ).GetUserData();
        double corr = (0+n_elet-1)*n_elet/2;
        if (tmp!=corr) {

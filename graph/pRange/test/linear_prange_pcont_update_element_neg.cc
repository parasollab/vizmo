...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer UpdateElement negative with " 
         << nThreads << " threads... ";
  }
  ...
  vec2.UpdateElement(x, negate<int>());
  vec2.UpdateElement(0, x);
  x = vec2.UpdateElement(0, negate<int>());

  pr.UpdateElement(x, negate<int>());
  pr.UpdateElement(0, x);
  x = pr.UpdateElement(0, negate<int>());

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer local_rbegin positive with " 
         << nThreads << " threads... ";
  }
  ...
  pvector::reverse_iterator i = vec1.local_rbegin();

  pvector::reverse_iterator j = pr.local_rbegin();


  bool corr_iter1 = (i == j);
  bool corr_iter2 = (&(*j) == &(*(pr.get_subranges()[0].get_boundary().finish())));
  bool corr_val   = (*j == *(vec1.local_rbegin()));


  // Report whether or not the test passed
  stapl_bool test_result(corr_iter1 && corr_iter2 && corr_val);
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer local_size positive with " 
         << nThreads << " threads... ";
  }
  ...
  int pc_size = vec2.local_size();

  int pr_size = pr.container_local_size();


  // Report whether or not the test passed
  stapl_bool test_result((pc_size == pr_size) && (pr_size == num_local_elems));
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer InitBoundaryInfo negative with " 
         << nThreads << " threads... ";
  }
  ...
  pr.InitBoundaryInfo(x, l, l);

  pr.InitBoundaryInfo(0, x, l);
  
  pr.InitBoundaryInfo(0, l, x);

  vec2.InitBoundaryInfo(x, l, l);

  vec2.InitBoundaryInfo(0, x, l);
  
  vec2.InitBoundaryInfo(0, l, x);

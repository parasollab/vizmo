...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer push_back positive with " 
         << nThreads << " threads... ";
  }
  ...
  bool corr_pr1 = (*(pr.get_subranges()[0].get_boundary().finish()) == 99);
  bool corr_pr2 = (*(vec1.local_end() - 1) == 99);
  bool corr_pc = (*(vec2.local_end() - 1) == 99);

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "graph boundary at_start positive with " 
         << nThreads << " threads... ";
  }
  ...
  // Test 1: call at_start with the iterator that is the start iterator.
  boundary::iteratorType i = bound.start();
  bool is_start = bound.at_start(i);

  // Test 2: call at_start with the iterator that is not the start iterator.
  ++i;
  bool not_start = bound.at_start(i);


  // Report whether or not the test passed
  stapl_bool test_result(is_start && (!not_start));
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange get_gid_producer positive with " 
         << nThreads << " threads... ";
  }
  ...
  global_id& gp = pr.get_gid_producer();

  bool corr_offset = (gp.get_offset() == 10000);
  bool corr_id = (gp.get() == 10000 * id + 2);

  // Report whether or not the test passed
  stapl_bool test_result(corr_offset && corr_id);
  test_result.reduce();

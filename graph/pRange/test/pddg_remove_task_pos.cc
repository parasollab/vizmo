...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "pDDG remove_task positive with " 
         << nThreads << " threads... ";
  }
  ...
  if (id == 0) {
    ...
    vector<int>::iterator ans1 = res1.end();
    --ans1;

    vector<int>::iterator ans2 = res2.end();
    --ans2;

    correct_values = ((*ans1 == 99) && (*ans2 != 99));
  } else {
    correct_values = true;
  }
  ...
  stapl_bool test_result(correct_values);
  test_result.reduce();

  if (id == 0) {
    if (test_result.value() && correct_values)
      cout << "Passed\n";
    else
      cout << "Failed\n";
  }

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange set_ddg positive with " 
         << nThreads << " threads... ";
  }
  ...
  prange::DDGType* g = new prange::DDGType(); // A partial order that organizes 
                                              // the elements into a 2d mesh.

  prange::DDGType* l = new prange::DDGType(); // A linear partial order of the 
                                              // elements.
  ...
  pr.set_ddg(l);
  prange::DDGType& new_ddg = pr.get_ddg();

  bool eq_linear = (new_ddg == l);

  pr.set_ddg(g);
  new_ddg = pr.get_ddg();

  bool eq_mesh = (new_ddg == g);


  // Report whether or not the test passed
  stapl_bool test_result(eq_linear && eq_mesh);
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "counted boundary at_finish negative with " 
         << nThreads << " threads... ";
  }
  ...
  // Test 1: call at_finish with non-iterator as its argument.
  bound.at_finish(x);

  // Test 2: call at_finish and store the return value in a non-boolean.
  x = bound.at_finish(vec.local_end() - 1);


  // Report whether or not the test passed
  stapl_bool test_result; //insert test for correctness in constructor argument
  test_result.reduce();

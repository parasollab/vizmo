...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange set_elem_ddg positive with " 
         << nThreads << " threads... ";
  }
  ...
  prange::ElemDDGType& new_elem_ddg = pr.get_element_ddg();

  bool eq_linear = (new_elem_ddg == *l);

  pr.set_element_ddg(g);
  new_elem_ddg = pr.get_element_ddg();

  bool eq_mesh = (new_elem_ddg == *g);

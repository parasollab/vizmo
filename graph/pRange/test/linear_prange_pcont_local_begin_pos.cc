...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer local_begin positive with " 
         << nThreads << " threads... ";
  }
  ...
  pvector::iterator i = vec1.local_begin();

  pvector::iterator j = pr.local_begin();


  bool corr_iter1 = (i == j);
  bool corr_iter2 = (j == pr.get_subranges()[0].get_boundary().start());
  bool corr_val   = (*j == *(vec1.local_begin()));

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange boundary constructor negative with " 
         << nThreads << " threads... ";
  }
  ...
  bool corr_boundary1 = (pr.get_subranges()[0].get_boundary() == b);
  bool corr_offset1 = (pr.get_gid_producer().get_offset() == 10000);
  bool corr_subranges1 = (pr.get_subranges().size() == 1);

  prange pr2(b, 99, &b);

  bool corr_boundary2 = (pr2.get_boundary() == b);
  ...
  stapl_bool test_result(corr_boundary1 && corr_offset1 && corr_subranges1 &&
                         corr_boundary2 && corr_offset2 && corr_subranges2);
  test_result.reduce();

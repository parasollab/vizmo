...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer FindRemotePid positive with " 
         << nThreads << " threads... ";
  }
  ...
  int pr_pid = pr.FindRemotePid(0);

  int pc_pid = vec2.FindRemotePid(0);



  // Report whether or not the test passed
  stapl_bool test_result((pr_pid == pc_pid) && (pr_pid == 0));
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  int midpoint = num_local_elems / 2;

  // Initial output to show what test is running
  if (id == 0) {
    cout << "Linear pRange add_subrange positive with " 
         << nThreads << " threads... ";
  }
  ...
  boundary b1(vec.local_begin(), vec.local_begin() + midpoint);
  boundary b2(vec.local_begin() + midpoint, vec.local_end() - 1);
  prange pr(&vec, b1);

  prange::subrangeType sr(b2);
  pr.add_subrange(sr);

  bool corr_num_subranges = (pr.get_num_subranges() == 2);
  bool corr_local_size = (pr.local_size() == num_local_elems);

  // Report whether or not the test passed
  stapl_bool test_result(corr_num_subranges && corr_local_size);
  test_result.reduce();

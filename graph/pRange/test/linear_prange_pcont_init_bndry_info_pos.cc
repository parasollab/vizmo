...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer InitBoundaryInfo positive with " 
         << nThreads << " threads... ";
  }
  ...
  pr.InitBoundaryInfo(0, loc, loc);

  vec2.InitBoundaryInfo(0, loc, loc);

  pair<Location, Location> pr_info = vec1.GetDistribution().GetPartBndry(0);
  pair<Location, Location> pc_info = vec2.GetDistribution().GetPartBndry(0);

  bool corr_pr = (pr_info.first.pid == id + 1) && (pr_info.first.part_id == 0) && (pr_info.second.pid == id + 1) && (pr_info.second.part_id == 0);
  bool corr_pc = (pc_info.first.pid == id + 1) && (pc_info.first.part_id == 0) && (pc_info.second.pid == id + 1) && (pc_info.second.part_id == 0);

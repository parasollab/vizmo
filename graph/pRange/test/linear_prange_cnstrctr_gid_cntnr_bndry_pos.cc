...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange GID/container/boundary constructor negative with " 
         << nThreads << " threads... ";
  }
  ...
  bool corr_id1 = (pr1.get_subranges()[0].get_global_id() == 99);
  bool corr_bound1 = (pr1.get_subranges()[0].get_boundary() == b);
  bool corr_elem1 = (pr1.GetElement(0) == vec.GetElement(0));
  bool corr_offset1 = (pr1.get_gid_producer().get_offset() == 10000);
  bool corr_subranges1 = (pr1.get_subranges().size() == 1);

  prange pr2(99, &vec, b, 99, &vec);

  bool corr_id2 = (pr2.get_global_id() == 99);
  bool corr_bound2 = (pr2.get_boundary() == b);
  bool corr_elem2 = (pr2.GetElement(0) == vec.GetElement(0));
  ...
  stapl_bool test_result(corr_id1 && corr_bound1 && corr_elem1 &&
                         corr_offset1 && corr_subranges1 &&
			 corr_id2 && corr_bound2 && corr_elem2 &&

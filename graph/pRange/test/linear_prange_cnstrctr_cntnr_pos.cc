...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange container constructor positive with " 
         << nThreads << " threads... ";
  }
  ...
  int pr_elem = pr.GetElement(0);

  bool corr_elem = (cont_elem == pr_elem);
  bool corr_offset = (pr.get_gid_producer().get_offset() == 10000);

  // Report whether or not the test passed
  stapl_bool test_result(corr_elem && corr_offset);
  test_result.reduce();

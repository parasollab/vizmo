...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "counted boundary finish modifier positive with " 
         << nThreads << " threads... ";
  }
  ...
  bound.finish(vec.local_end() - 2);
  bool finish_not_changed = *bound.finish() == *(vec.local_end() - 1);
  bool finish_changed = *bound.finish() == *(vec.local_end() - 2);


  // Report whether or not the test passed
  stapl_bool test_result(!finish_not_changed && finish_changed); 
  test_result.reduce();

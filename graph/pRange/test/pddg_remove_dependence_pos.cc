...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "pDDG remove_dependence positive with " 
         << nThreads << " threads... ";
  }
  ...
  g.add_dependence(1, 0);

  g.initialize_base_num_predecessors();
  g.reset();

  vector<int> first_call = g.get_ready();

  stapl_vector<int> ready1;
  for (vector<int>::iterator i = first_call.begin(); i != first_call.end(); ++i)
    ready1.value().push_back(*i);
  vector<int> res1 = ready1.combine();

  g.remove_dependence(1, 0);

  rmi_fence();
  g.reset();

  vector<int> second_call = g.get_ready();

  stapl_vector<int> ready2;
  for (vector<int>::iterator i = second_call.begin(); i != second_call.end(); ++i)
    ready2.value().push_back(*i);
  vector<int> res2 = ready2.combine();


  bool correct_values1(false);
  ...
  if (id == 0) {
    ...
    correct_values1 = equal(res1.begin(), res1.end(), ans1.begin());
    correct_values2 = equal(res2.begin(), res2.end(), ans2.begin());
  } else {
    correct_values1 = true;
    correct_values2 = true;
  }

  
  // Report whether or not the test passed
  stapl_bool test_result(correct_values1 && correct_values2);
  test_result.reduce();

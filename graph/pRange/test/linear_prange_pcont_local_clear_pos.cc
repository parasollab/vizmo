...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer local_clear positive with " 
         << nThreads << " threads... ";
  }
  ...
  bool corr_size, corr_loc_size, corr_num_sranges;

  if (id == 0) {
    corr_size = ((pr.container_size() == vec2.size()) && (pr.container_size() == 0));
    corr_loc_size = ((pr.container_local_size() == vec2.local_size()) && (pr.container_local_size() == 0));
    corr_num_sranges = (pr.get_num_subranges() == 0);
  } else {
    corr_size = ((pr.container_size() == vec2.size()) && (pr.container_size() != 0));
    corr_loc_size = ((pr.container_local_size() == vec2.local_size()) && (pr.container_local_size() != 0));
    corr_num_sranges = (pr.get_num_subranges() != 0);

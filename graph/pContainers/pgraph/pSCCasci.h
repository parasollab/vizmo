...
class pSCCasci: public BasePObject{
 ...
 public:
  pSCCasci(bool sccreuse=false, bool localcc=true, bool rbm=false){
    OLDSCCREM = sccreuse;
    LCCMULTI  = localcc;
  ...
  void collect(vector<map<VID,vector<int> > >& SCC, int nrtr, int maxsz){
	...
	for(scci=SCC[k].begin();scci != SCC[k].end();scci++){
	  stapl::async_rmi( 0, 
			    this->getHandle(),
			    &pSCCasci<PGRAPH>::_collect,k,*scci); 
	}
	//printf("\n");
      }
    }
  ...
  void dump(char* file_name,int tr){
	...
	if(scci->second.size() > 1){
	  for(int j=0;j<scci->second.size();j++){
	    _myistream<<scci->second[j]<<" "<<scci->first<<endl;
	  }
	}
      }
      _myistream.close();
  ...
  int compactSCC(vector<trimer<PGRAPH>*>&  _trimers, vector<vector<pair<VID,int> > >& SCC){
   ...
   for(i=0;i<_trimers.size();i++){
	 ...
	 stapl::async_rmi(_n.locpid(),this->getHandle(),&pSCCasci<PGRAPH>::_compactSCC,arg);
       }//else remote
  ...
  int compactSCCLeader(int _trs, vector<vector<pair<VID,int> > >& SCC){
   ...
   for(i=0;i < _trs;i++){
	 ...
	 //trimer_id, SCC_id, vertex_id
	 _StaplTriple<int,VID,VID> arg(i,vit->second, vit->first);
	 stapl::async_rmi(0,this->getHandle(),&pSCCasci<PGRAPH>::_compactSCCLeader,arg);
       }//else remote
     }//for every vertex in pSCC
   }//for every trimer
  ...
  int mark_method(vector<trimer<PGRAPH>* >& _trimers,bool reuse){
    ...
    stapl::rmi_fence();
    stapl::rmi_fence();
    stapl::reduce_rmi<pSCCasci, int>(&finished, 
				 &global, 
				 getHandle(),
				 &pSCCasci::_MIN, 
				 true);
	...
	for(vi = _trimers[tr_id]->sccg.local_begin();
	  ...
	  for(pivt=0;pivt<vi->data.forward.size();pivt++){
	    ...
	    if((vi->data.forward[pivt] == vi->data.back[pivt]) 
	       && (vi->data.back[pivt] != -1)){
	      scci = SCC[tr_id].find(vi->data.forward[pivt]);
	      if (scci == SCC[tr_id].end()){
		//new scc
		vector<int> temp;
		temp.push_back(vi->vid);
		SCC[tr_id][vi->data.forward[pivt]] = temp;
		//cout<<"SCC considered:"<<vi->vid<<":"<<vi->data.forward[pivt]<<":"<<pivt<<endl;
	      }
	      else{
		//some vertices of scc are there
		scci->second.push_back(vi->vid);
		//cout<<"2_SCC considered:"<<vi->vid<<":"<<vi->data.forward[pivt]<<":"<<pivt<<endl;
	      }
	  ...
	  if(LCCMULTI){
	    ...
	    if(pf != NULL){
	      if(vi->data.forward[myid] != -1 && vi->data.back[myid] != -1 && vi->data.forward[myid] == vi->data.back[myid]){
		mit = scc_ccno.find(vi->data.ccno);
		if(mit == scc_ccno.end()){
		  scc_ccno[vi->data.ccno] = CCNO + 1;
		  vi->data.ccno = CCNO + 1;
		  CCNO++;
		}
		else{
	  ...
	  for(j=0;j<neighbours.size();j++){
		...
		if(data2.back[k] == -1 && data2.forward[k] != -1){
	      ...
	      if(pf != NULL){
		//break between current SCC and the rest
		if(data1.back[k] != -1 && data1.forward[k] != -1 ){
		  if(data2.back[k] == -1 || data2.forward[k] == -1){
		    del_edgef.push_back(neighbours[j]);
		  }

...
public:
  ...
  //@{
    ...
    EI my_find_pred_EID2_eq(EI _start, EI _end, const EID _eid) ; 
  ...
  vector<STAPL_GRAPH_COLOR> in;
  vector<STAPL_GRAPH_COLOR> out;
  /**
   *Sets the maximum number of simultaneous traversals
  ...
  STAPL_GRAPH_COLOR GetColorIn(int _tr_id){
    assert(_tr_id < in.size());
    return in[_tr_id];
  }
  STAPL_GRAPH_COLOR GetColorOut(int _tr_id){
    assert(_tr_id < out.size());
    return out[_tr_id];
  }
  STAPL_GRAPH_COLOR SetColorIn(int _tr_id,STAPL_GRAPH_COLOR _color){
    assert(_tr_id < in.size());
    return in[_tr_id]=_color;
  }
  STAPL_GRAPH_COLOR SetColorOut(int _tr_id,STAPL_GRAPH_COLOR _color){
    assert(_tr_id < out.size());
  ...
  int ByteSize(){
    ...
    //cout<<"EDGELISTSIZE:"<<sizeof(edgelist[0])<<":"<<sizeof(double)<<endl;
...
public AbstractGraph<VERTEX> {
 public:
  ...
  //////////////////////////////////////////////////////////////////////////////////////////
	...
	BaseGraph<VERTEX,WEIGHT>& operator=(BaseGraph<VERTEX,WEIGHT>const &j) {
	   if (this != &j) {
             this->v = j.v;
#ifdef DYNAMIC_GRAPH
   	     this->gid_localid_map = j.gid_localid_map;
#endif 
           }
	   return *this;
        }
  ...
  //////////////////////////////////////////////////////////////////////////////////////////
    ...
    //* only used internally in IsVertex function
      //typedef map<VID,int> LOOKUPTABLE;
      typedef hash_map<VID,int> LOOKUPTABLE;
      LOOKUPTABLE gid_localid_map;
      void resizeLookupTable(int newsize){
	gid_localid_map.resize(newsize);
      }
      void printLookupTable(){
	cout<<"LOOKUPTABLE::"<<gid_localid_map.bucket_count()<<endl;;
      }
#endif
  ...
  //===========================
    const LOOKUPTABLE& GetLocalIDMap() const { 
      return gid_localid_map;
    }

    void SetLocalIDMap(const LOOKUPTABLE& _m) { 
      gid_localid_map = _m;
    }
    ...
    bool IsInLocalIDMap(VID _gid,  LOOKUPTABLE::iterator& _it) {
    ...
    void DisplayVIDLocalIDMap(){
      for(LOOKUPTABLE::iterator it=gid_localid_map.begin(); 
...
my_find_VID_eq(const VID _vid)  {
    ...
    //cout<<"--- "<<_vid<<endl;
    LOOKUPTABLE::const_iterator mit = gid_localid_map.find(_vid);
...
int 
WtVertexType<VERTEX,WEIGHT>::
DeleteXPredEdges(VID _v2id, int _x) {
    int num_to_delete; 
    int num_deleted = 0;
    if (_x == -1) 
        num_to_delete = predecessors.size();
    else num_to_delete = _x;
    EI ei = my_find_pred_EID1_eq(_v2id);
    while ( (ei != predecessors.end()) ) {
      predecessors.erase(ei);
      /*
      if(ei + 1 == predecessors.end()) predecessors.pop_back();
      else {
	*ei = *(predecessors.end() - 1);
	predecessors.pop_back();
      }
      */
      num_deleted++;
      if (num_deleted >= num_to_delete) break;
      ei = my_find_pred_EID2_eq(ei,predecessors.end(),_v2id);
    }
    return num_deleted;
}
...
DisplayEdgelist(int wt) const {
    ...
    cout << "} ";
#ifndef NOPREDECESSORS 
    cout<<"Predecessors={";

    for (ei = predecessors.begin(); ei < predecessors.end(); ei++) {
        ei->DisplayEdge(wt);
...
typename  vector<WtEdgeType<VERTEX,WEIGHT> >::const_iterator
WtVertexType<VERTEX,WEIGHT>::
my_find_pred_EID1_eq(const EID _eid) const {
    CEI ei = predecessors.begin();
    bool found = false;
    while (ei != predecessors.end() && !found) {
        if ( ei->vertex2id == _eid) {
            found = true;
        } else {
            ei++;
...
typename  vector<WtEdgeType<VERTEX,WEIGHT> >::iterator
WtVertexType<VERTEX,WEIGHT>::
my_find_pred_EID1_eq(const EID _eid) {
    EI ei = predecessors.begin();
    bool found = false;
    while (ei != predecessors.end() && !found) {
        if ( ei->vertex2id == _eid) {
            found = true;
...
typename  vector<WtEdgeType<VERTEX,WEIGHT> >::iterator
WtVertexType<VERTEX,WEIGHT>::
my_find_pred_EID2_eq(EI _start, 
                EI _end,
                const EID _eid) {
    EI ei = _start;
    bool found = false;

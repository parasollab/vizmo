...
/* 	return; */
  ...
  int GetOutgoingEdgesVData(VID _v1id, vector< pair<VERTEX,VERTEX> >& iedges) const{
	...
	iedges.push_back( nextedge );
      ...
      cout << "\nGetOutgoingEdgesVData: vertex "<< _v1id << " not in graph";
    }
    return iedges.size();
  }
...
class Graph:public D, public M, public W{
  ...
  Graph<D,M,W,VERTEX,WEIGHT>& operator=(Graph<D,M,W,VERTEX,WEIGHT>const &j) {
           if (this != &j) {
             this->v = j.v;
// The value of the member numEdges has to be explicitly copies otherwise future GetEdgeCount method will simply return the old value of numEdges
	     this->numEdges = j.numEdges;
#ifdef DYNAMIC_GRAPH
   ...
   *Doublely vertices listed in given vector<VID> with various weight.
   *If path contains (vid0, vid1,vid2,vid3,...), then
...
//added by thuang - to handle edges like 8-8, 9-9... in UG
    int equaledges = 0;
...
//added by thuang - to handle edges like 8-8, 9-9... in UG
   ...
   *@see DeleteWtEdge(VID, VID, WEIGHT, int _n) and AddEdge(VID, VID, WEIGHT)
   */
  int ChangeEdgeWeight(VID _v1id, VID _v2id, pair<WEIGHT,WEIGHT>& _weight) {
    //this is only for weighted, undirected  one;
    //something such that the compiler will generate errors
    //or return -1 immediate
    //and I think this can be done faster; and moved in weighted class!!!
    VI vi;
    EI ei;
    if(!W::check_weighted()) return ERROR;
    if(this->IsDirected()) return ERROR;
    
    if(W::IsEdge(_v1id, _v2id, &vi,&ei)){
      ei->weight = _weight.first;
    }
    else return ERROR;
    if(W::IsEdge(_v2id, _v1id, &vi,&ei)){
      ei->weight = _weight.second;
    }
    else return ERROR;
    return OK;
  }
   ...
   *AddEdge(VERTEX&, VERTEX&, WEIGHT)
   */
  int ChangeEdgeWeight(VERTEX& _v1, VERTEX& _v2, pair<WEIGHT,WEIGHT>& _weight) {
    VI vi;
    EI ei;
    if(!W::check_weighted()) return ERROR;
    if(this->IsDirected()) return ERROR;
  ...
  ReadExoGraph(istream& _myistream) {
    ...
    cout<<"Vertices:"<<nVerts<<" Edges:"<<nEdges<<" maxVID:"<<maxVID<<endl;
    ...
    for (i = 0; i < nVerts; i++){
      ...
      //cout<<"read vertex:"<<v1id;

      _myistream>>tr1>>dtr1>>dtr2>>dtr3>>tr2;
      //_myistream >> tagstring;
      //cout<<" "<<tagstring<<endl;
      //_myistream >> tagstring;
      //cout<<" "<<tagstring<<endl;
   ...
   * the graphs are from Yu-Kwong Kwok's graph benchmarks
   * with slightly different format. No "GRAPH.." tags.
   * vertex id starts from 1. It is converged to 0 in the following method.
   */
  void ReadBenchMarkGraph(istream& _myistream) {
    VID v1id, v2id, maxVID;
    VI  v1;
    VERTEX data;
    WEIGHT weight;
    int nVerts, nEdges=0, nedges;
    ...
    this->vertIDs = maxVID; // set the maximum VID used so far...
    // should sort verts & find biggest used...
/*     cout<<"nverts:"<<nVerts<<" nEdges:"<<nEdges
       <<" maxVID:"<<maxVID<<endl; */
  }

  /**Read graph info from the file of file name, _filename.
   *This method calls ReadBenchMarkGraph(istream& _myistream).
   */
  void ReadBenchMarkGraph(const char*  _fname) {
    ifstream  myifstream(_fname);
    if (!myifstream) {
      cout << "\nIn ReadGraph: can't open infile: " << _fname ;
      return;
    }
    ReadBenchMarkGraph(myifstream);
    myifstream.close();
  }

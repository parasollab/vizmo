...
public:
  ...
  //////////////////////////////////////////////////////////////////////////////////////////
    ...
    //@{
       ...
       void ReadDotGraph(const char*  _filename);
...
ReadDotGraph(istream& _myistream) {
    ...
    int nVerts=0, nEdges=0, nedges=0;
    char tagstring[100];
    _myistream >> tagstring;
    if ( !strstr(tagstring,"GRAPHSTART") ) {
        cout << endl << "In ReadGraph: didn't read GRAPHSTART tag right";
...
ReadDotGraph(istream& _myistream) {
    ...
    while(v1id != -1){
      if ( !IsVertex(v1id,&cv1) ) {
            cout << "\nIn ReadGraph:add v1 didn't exist...";
        }
      v1 = const_cast<VI>(cv1);
      _myistream >> v2id;
      ...
      //this is a shorctut to test undirectness
      //if ( !IsVertex(v2id,&cv1) ) {
      //      cout << "\nIn ReadGraph:add v2 didn't exist...";
      //  }
      //v1 = const_cast<VI>(cv1);
      //v1->AddEdge(v1id,weight);
      //nEdges++;
...
#else
    //sort ( ccstats.begin(), ccstats.end(), __CCVID_Compare<pair<int,VID> >() );

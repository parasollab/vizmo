...
/*
       ...
       if(initString[0]=='#')  {
        ...
        _is >> initString;
        _is >> initString;
       _is >> noOfActors;      // # of MultiBodys'
       }
       else {
         sscanf(initString,"%d",&noOfActors);
       }
       cout << "# bodies=" << noOfActors << endl;
       for (int m=0; m<noOfActors; m++){
        sprintf(objectName,"Noname");
        color[0]=color[1]=color[2]=0.9;
	solidFlag=1;
        //---------------------------------------------------------------
        // Read tag
        //---------------------------------------------------------------
        vizReadField(_is, &string);              // Tag, "MultiBody"
        cout << "Tag " << string << " ";
...
/*
            ...
            _is >> color[2];
*/
            sscanf(string,"%s %f %f %f",temps,&color[0],&color[1],&color[2]);
            cout << "Color " << color[0] << " " << color[1] << " " << color[2] << endl;
            vizReadField(_is, &string);
        }
        if( !strncmp(string,"#VIZMO_NAME",11))
        {
            //_is >> objectName;
            
            sscanf(string,"%s %s",temps,objectName);
             cout << "ObjectName " << objectName<< endl;
            vizReadField(_is, &string);

        }
        if( !strncmp(string,"#VIZMO_SOLID",12))
        {
            //_is >> solidFlag; 
            sscanf(string,"%s %d",temps,&solidFlag);
            vizReadField(_is, &string);
        }


        _is >>  j; // Tag for body count in body , discard
        _is >> temps;
        if(  temps[0]=='/')
            sprintf(objectFile,"%s",temps);
         else
            sprintf(objectFile,"%s/%s",path,temps);
        cout << string << " " << j << " " << objectFile << endl;
        ...
        if (!invalidFile) {
           ...
           if (strncmp(string, "FreeBody", 9))
                 tempActor->setObstacle();
           else  tempActor->setRobot();
           tempActor->setSolid();

           _is >> tempActor->part.conf[0];
           _is >> tempActor->part.conf[1];
           _is >> tempActor->part.conf[2];
           _is >> tempActor->part.conf[3];
           _is >> tempActor->part.conf[4];
           _is >> tempActor->part.conf[5];

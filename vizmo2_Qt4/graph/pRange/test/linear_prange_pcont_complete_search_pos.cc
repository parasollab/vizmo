...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer CompleteSearch positive with " 
         << nThreads << " threads... ";
  }
  ...
  Location pr_loc = pr.CompleteSearch(0);
  Location pc_loc = vec2.CompleteSearch(0);


  // Report whether or not the test passed
  stapl_bool test_result((pr_loc.pid == 0) && (pr_loc.part_id == 0) &&
		         (pc_loc.pid == 0) && (pc_loc.part_id == 0));
  test_result.reduce();

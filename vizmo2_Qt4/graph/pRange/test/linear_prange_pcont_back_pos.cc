...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer back positive with " 
         << nThreads << " threads... ";
  }
  ...
  pvector::iterator pr_it1 = pr.back();
  pvector::const_iterator pr_it2 = pr.back();

  pvector::iterator pc_it1 = vec2.back();
  pvector::const_iterator pc_it2 = vec2.back();

  int pr_val2 = *pr_it2;
  int pc_val2 = *pc_it2;
  bool pr_corr = ((*pr_it1 == *pr_val2) && (*pr_it1 == *(vec1.local_begin())));
  bool pc_corr = ((*pc_it1 == *pc_val2) && (*pc_it1 == *(vec2.local_begin())));

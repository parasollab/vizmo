...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange set_boundary positive with " 
         << nThreads << " threads... ";
  }
  ...
  b.start(b.start() + 1);
  pr.get_subranges()[0].set_boundary(b);

  bool corr_size = (pr.local_size() == vec.local_size() - 1);
  bool corr_elem = (*(pr.get_subranges()[0].get_boundary().start()) == vec.GetElement(num_local_elems * id + 1));

  // Report whether or not the test passed
  stapl_bool test_result(corr_size && corr_elem);
  test_result.reduce();

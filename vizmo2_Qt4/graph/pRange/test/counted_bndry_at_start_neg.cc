...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "counted boundary at_start negative with " 
         << nThreads << " threads... ";
  }
  ...
  // Test 1.  Call at_start with non-iterator argument.
  bound.at_start(x);

  // Test 2.  Call at_start and store the return value in a non-boolean.
  x = bound.at_start(vec.local_begin());

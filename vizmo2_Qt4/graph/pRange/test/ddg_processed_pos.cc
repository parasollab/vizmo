...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "DDG processed positive with " 
         << nThreads << " threads... ";
  }
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    sort(ready_set.begin(), ready_set.end());

    //If a previous iteration hasn't failed then check the result.
    if (correct_ans && (id == 0)) {
      correct_ans = equal(answer[i].begin(), answer[i].end(), ready_set.begin());
      if (!correct_ans) {
	cout << "iteration " << i << ": ";
  ...
  //If a previous iteration hasn't failed then check the result.
  if (correct_ans && (id == 0)) {
    correct_ans = equal(answer[5].begin(), answer[5].end(), ready_set.begin());
    if (!correct_ans) {
      cout << "iteration 5: ";
      for (vector<int>::iterator j = ready_set.begin(); j != ready_set.end(); ++j)
        cout << *j << " ";

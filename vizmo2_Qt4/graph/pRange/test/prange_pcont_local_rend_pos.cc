...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "pRange pContainer local_rend positive with " 
         << nThreads << " threads... ";
  }
  ...
  pvector::reverse_iterator i = vec1.local_rend();

  pvector::reverse_iterator j = pr.local_rend();


  bool corr_iter1 = (i == j);
  pvector::reverse_iterator k(pr.get_subranges()[0].get_boundary().start() - 1);
  bool corr_iter2 = (j == k);

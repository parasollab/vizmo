...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "pDDG finished positive with " 
         << nThreads << " threads... ";
  }
  ...
  stapl_vector<int> num_processed;
  num_processed.value().push_back(0);

  while (!g.finished()) {
    vector<int> ready = g.get_ready();
    for (vector<int>::iterator i = ready.begin(); i != ready.end(); ++i) {
      g.processed(*i);
  ...
  num_processed.reduce();

  bool correct(true);
  if (id == 0) {
    correct = false;
    correct = (num_processed.value()[0] == 64);
  }

  // Report whether or not the test passed
  stapl_bool test_result(correct);
  test_result.reduce();

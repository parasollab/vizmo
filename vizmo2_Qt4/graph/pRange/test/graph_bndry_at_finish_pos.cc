...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "graph boundary at_finish positive with " 
         << nThreads << " threads... ";
  }
  ...
  // Test 1: call at_finish with an iterator pointing to the last element.
  boundary::iteratorType i = bound.finish();
  bool is_finish = bound.at_finish(i);

  // Test 2: call at_finish with an iterator not pointing to the last element.
  i = bound.start();
  bool not_finish = bound.at_finish(i);


  // Report whether or not the test passed
  stapl_bool test_result(is_finish && (!not_finish));
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer local_empty positive with " 
         << nThreads << " threads... ";
  }
  ...
  bool pc_empty = vec2.local_empty();
  bool pr_empty = pr.local_empty();
  bool corr1 = (!pc_empty && !pr_empty);
  rmi_fence();
  ...
  pc_empty = vec2.local_empty();
  pr_empty = pr.local_empty();
  bool corr2 = (pc_empty && pr_empty);

  // Report whether or not the test passed
  stapl_bool test_result(corr1 && corr2);
  test_result.reduce();

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer insert negative with " 
         << nThreads << " threads... ";
  }
  ...
  pr.insert(pr.get_boundary().start(), x);

  x = pr.insert(pr.get_boundary().start(), 99);

  vec2.insert(x);

  vec2.insert(vec2.local_begin(), x);

  x = vec2.insert(vec2.local_begin(), 99);

...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange get_container_version positive with " 
         << nThreads << " threads... ";
  }
  ...
  int pr_ver = pr.get_container_version();
  int pc_ver = vec.getDistributionVersion();


  // Report whether or not the test passed
  stapl_bool test_result(pr_ver == pc_ver);
  test_result.reduce();

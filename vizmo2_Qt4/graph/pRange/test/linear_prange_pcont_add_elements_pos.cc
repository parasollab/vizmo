...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange pContainer AddElements positive with " 
         << nThreads << " threads... ";
  }
  ...
  vector<int> elems(nThreads);
  sequence seq3(1);
  generate(elems.begin(), elems.end(), seq3);

  pr.AddElements(elems);
  vec2.AddElements(elems);
  rmi_fence();

  stapl_int size(pr.local_size());
  size.reduce();

  bool corr_pr_size = (size.value() == 100 + nThreads*nThreads);
  bool corr_pc_size = (vec2.size() == 100 + nThreads*nThreads);
  bool corr_version = (pr.get_container_version() == vec1.version());


  // Report whether or not the test passed
  stapl_bool test_result(corr_pr_size && corr_pc_size && corr_version);
  test_result.reduce();

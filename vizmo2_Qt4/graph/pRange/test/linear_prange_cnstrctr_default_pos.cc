...
void stapl_main(int argc, char **argv) {
  ...
  if (id == 0) {
    cout << "Linear pRange default constructor positive with " 
         << nThreads << " threads... ";
  }
  ...
  prange pr1;
  bool first_correct = (pr1.get_gid_producer().get_offset() == 10000);

  prange pr2(99);
  bool second_correct = (pr2.get_gid_producer().get_offset() == 99);


  // Report whether or not the test passed
  stapl_bool test_result(first_correct && second_correct);
  test_result.reduce();

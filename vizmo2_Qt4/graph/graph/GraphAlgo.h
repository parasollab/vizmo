 ...
 *@return A list of pairs. The first element in each pair is the size
...
*/
template <class GRAPH>
int FindPathDijkstra1(GRAPH& _g, VID _v1id, VID _v2id,
  ...
  while(parent[temp] != _v1id){
    typename GRAPH::VERTEX_TYPE tmp1 = _g.GetData(parent[temp]);
    typename GRAPH::WEIGHT_TYPE wt = _g.GetEdgeWeight(parent[temp],temp);
    _V.push_back(pair<typename GRAPH::VERTEX_TYPE,
...
int FindPathDijkstra(GRAPH& _g, VID _v1id, VID _v2id,
  ...
  typename GRAPH::WEIGHT_TYPE t1;
  _V.clear();
  status.resize(_g.GetNextVID());
  dist.resize(_g.GetNextVID());
  parent.resize(_g.GetNextVID());
...
class _visitor_SCC: public visitor_base<GRAPH>{
 ...
 public:  
  ...
  int finish_vertex(typename GRAPH::VI vi){
    (*finishtimes)[sz-finish-1].first  = vi->vid;
    (*finishtimes)[sz-finish-1].second = finish;
    finish++;
    return OK;
  }
...
template <class GRAPH>
int scc(GRAPH& _g, vector<vector<VID> >& _scc,bool predset=false){
  ...
  if(predset == false)  _g.SetPredecessors();

#include "pointerjumping_tester.h"
#include <assert.h>
#include <runtime.h>
...
public:
  ...
  void SetAllTesters() {
    Tester_PointerJumping* t0 = new Tester_PointerJumping();
...
//  if (nprocs == 1) {cout<<"It doesn't make sense to call Pointer Jumping on 1 thread. No work will be done. "<<endl; return; }

  bool flag = false;

  //read in argvs if "exe 10 -testall 50 " 
  //translate to be "exe 10 Tester_PointerJumping b 50"
  char *p; char *space=" ";
  ...
  TestManager tm;
  if (flag) {
    char* n_elet = argv[3]; argc = 5; argv[2]="Tester_PointerJumping";  argv[3]="b"; argv[4]=n_elet;

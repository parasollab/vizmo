...
namespace plum{
  ...
  bool CPolyhedronModel:: BuildModels() {
    ...
    bool isSurface = m_BodyInfo.m_IsSurface;
    //build solid model
    if( BuildGLModel_Solid(points,tris,com,normal,isSurface)==false ) {
      cerr<<"Build Model ("<<m_BodyInfo.m_strModelDataFileName<<") Error"<<endl;
      return false;
    }

    //build wired frame
    if( BuildGLModel_Wired(points,tris,com,normal,isSurface)==false ) {
      cerr<<"Build Model ("<<m_BodyInfo.m_strModelDataFileName<<") Error"<<endl;
      return false;
    }
  ...
  bool CPolyhedronModel::BuildGLModel_Wired
	...
	if(tri_l==-1 || tri_r==-1) 
          draw = true;
	else if((1-fabs(n[tri_l]*n[tri_r])) > 1e-3) 
          draw = true;

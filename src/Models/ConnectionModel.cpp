...
ConnectionModel::
GetJointTypeFromTag(const string& _tag) {
  if(_tag == "NONACTUATED")
    ...
    throw ParseException(WHERE, "Failed parsing joint type. "
        "Choices are Revolute or Spherical.");
}
...
operator>>(istream& _is, ConnectionModel& _c) {
  //body indices
  _c.m_previousIndex = ReadField<size_t>(_is, WHERE,
      "Failed reading previous body index");
  _c.m_nextIndex = ReadField<size_t>(_is, WHERE,
  ...
  if(_c.m_jointType == ConnectionModel::REVOLUTE
      || _c.m_jointType == ConnectionModel::SPHERICAL) {
    ...
    for(size_t i = 0; i<numRange; i++) {
      ...
      if(_is >> tok) {
        ...
        if(!(minv >> _c.m_jointLimits[i].first
            && maxv >> _c.m_jointLimits[i].second))
          throw ParseException(WHERE, "Failed reading joint range.");
      }
      else if(numRange == 2 && i == 1) //error. only 1 token provided.
        throw ParseException(WHERE, "Failed reading joint ranges. "
            "Only one provided.");
    }
  }

  //Transformations and DH parameters
  _c.m_toDHFrame = ReadField<Transformation>(_is, WHERE,
      "Failed reading transformation to DHFrame");
  Vector4d dhparameters = ReadField<Vector4d>(_is, WHERE,
      "Failed reading DH Parameters.");
  _c.m_alpha = dhparameters[0];
  _c.m_a = dhparameters[1];
  _c.m_d = dhparameters[2];
  _c.m_theta = dhparameters[3];
  _c.m_toBody2 = ReadField<Transformation>(_is, WHERE,
      "Failed reading transformation to body 2");

...
obprmGUI::obprmGUI( const QString name, QWidget *parent)
: QDialog(parent)
{
...
tabWidget->addTab(new QLabel(tr("Test")), tr("General"));
...
connect(okButton, SIGNAL(clicked()), this,  SLOT(makeCommandLine()));
...
void obprmGUI:: getOBPRMpath(){
    ...
    Q3FileDialog* fd = new Q3FileDialog( this, "obprm_dialog", TRUE );
...
QGridLayout *grid = new QGridLayout;
  ...
  grid->addWidget( new QLabel( "Filename:"), 0, 0);
  ...
  QRadioButton * r1 = new QRadioButton("Other");
  connect( r1, SIGNAL(clicked()), this, SLOT(showFileDialog()) );
  grid->addWidget( r1, 1, 0);

  cb_partEdges = new QCheckBox("-addPartialEdge");
  grid->addWidget(cb_partEdges, 2, 0 );
  addPartEdge = new QLineEdit;
  addPartEdge->setText("0");
  grid->addWidget(addPartEdge, 2, 1 );

  cb_allEdges = new QCheckBox("-addAllEdges");
  grid->addWidget(cb_allEdges, 3, 0);
  addAllEdgs = new QLineEdit;
  addAllEdgs->setText("0");
  grid->addWidget(addAllEdgs, 3, 1);

  cb_posRes = new QCheckBox("-posres");
  grid->addWidget(cb_posRes, 4, 0);
  posRes = new QLineEdit;
  posRes->setText("0.05");
  grid->addWidget(posRes, 4, 1);
  grid->addWidget(new QLabel("Position Resolution *calculated*"), 4, 2);

  cb_oriRes = new QCheckBox("-orires");
  grid->addWidget(cb_oriRes, 5, 0);
  oriRes = new QLineEdit;
  oriRes->setText("0.05");
  grid->addWidget(oriRes, 5, 1);
  grid->addWidget(new QLabel("Orientation Resolution *HARDCODED*"), 5, 2);
 
  cb_numJoints = new QCheckBox("-numofjoints");
...
void obprmGUI::setupTabBbox(){
  ...
  Q3Grid *grid = new Q3Grid(3, Qt::Vertical);
  ...
  QRadioButton * rb1 = new QRadioButton("Default");
  bGrp->insert(rb1, 0);
  rb1->setChecked( TRUE );
  connect( rb1, SIGNAL(clicked()), this, SLOT(getOption()) );

  QRadioButton * rb2 = new QRadioButton("Set");
  bGrp->insert(rb2, 1);
  connect( rb2, SIGNAL(clicked()), this, SLOT(showBoxes()) );

  addBoxes(grid);
  tabWidget->addTab(grid, "BBox");
...
// 				 Qt::AlignCenter);
  ...
  tabWidget->addTab(basicWidgt, "-gNodes");
...
{
    QString fn=Q3FileDialog::getOpenFileName(QString::null,"Map File (*.map)",this);
    if ( !fn.isEmpty() ){
...
{
    QString fn=Q3FileDialog::getOpenFileName(QString::null,"Env File (*.env)",this);
    if ( !fn.isEmpty() ){
...
void textEditDialog::createFile(){
  ...
  fn = Q3FileDialog::getSaveFileName( QString::null, tr( "All Files (*)" ), this );
  if ( !fn.isEmpty() ) {

...
inline void
gliDrawRotateAxis(Camera* camera) {
  static int gid=-1;
...
inline Point3d
gliProject2Window(const Point3d& pt) {
  //Get matrix info
...
inline void
gliProject2Window(Point3d* pts, int size) {
  //Get matrix info
  int ViewPort[4]; double ModelViewM[16], ProjM[16];
  glGetIntegerv( GL_VIEWPORT, ViewPort );
  glGetDoublev( GL_MODELVIEW_MATRIX, ModelViewM );
...
inline Point3d
gliUnProj2World(const Point3d& ref,const Vector3d& n,int x, int y) {
...
inline Point3d
gliUnProj2World(const Point3d& ref, int x, int y) {

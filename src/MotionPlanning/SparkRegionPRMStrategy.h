#ifndef SPARKREGIONPRMSTRATEGY_H_
#define SPARKREGIONPRMSTRATEGY_H_
...
class SparkRegionPRMStrategy : public MPStrategyMethod<MPTraits> {
	public:
		...
		SparkRegionPRMStrategy();
		SparkRegionPRMStrategy(MPProblemType* _problem, XMLNodeReader& _node);
...
template<class MPTraits>
SparkRegionPRMStrategy<MPTraits>::
SparkRegionPRMStrategy() {
	this->SetName("SparkRegionPRMStrategy");
}
...
Run() {
	...
	//start clock
	GetVizmo().StartClock("SparkRegionPRMStrategy");
	this->GetMPProblem()->GetStatClass()->StartClock("SparkRegionPRMStrategyMP");
	...
	//stop clock
	GetVizmo().StopClock("SparkRegionPRMStrategy");
	this->GetMPProblem()->GetStatClass()->StopClock("SparkRegionPRMStrategyMP");
}
...
Finalize() {
	...
	GetVizmo().PrintClock("SparkRegionPRMStrategy", cout);
	//stats->PrintClock("Pre-regions", cout);
	stats->PrintClock("SparkRegionPRMStrategyMP", cout);
	...
	GetVizmo().PrintClock("SparkRegionPRMStrategy", ostats);
	//stats->PrintClock("Pre-regions", ostats);
	stats->PrintClock("SparkRegionPRMStrategyMP", ostats);
...
SampleRegion(size_t _index, vector<CfgType>& _samples) {
	...
	/*	typedef vector<RegionModel*>::const_iterator RIT;
		vector<RegionModel*> attractRegions = GetVizmo().GetEnv()->GetAttractRegions();

		int num = _index;
		RIT regionToDelete = attractRegions.begin() + num;

		GetVizmo().GetEnv()->DeleteRegion(*regionToDelete);*/
	}

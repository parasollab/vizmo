...
namespace plum{
   ...
   template <class Cfg, class WEIGHT> class CCModel : public CCModelBase
   {
      ...
      void changeColor(float r, float g, float b, Shape s){ 
         ...
         //std::cout << "change Color called" << endl;
      ...
      void SetColor(float r, float g, float b, float a){ 
         ...
         //std::cout << "set Color called" << endl;
      ...
      void change_properties(Shape s, float size, float *color, bool Isnew){
         ...
         m_DID_PT=-1;    		
         if(Isnew)
            changeColor(color[0], color[1], color[2], s);
      ...
      void BuildNodeModels(GLenum mode);
   ...
   template <class Cfg, class WEIGHT>
      void CCModel<Cfg, WEIGHT>::BuildNodeModels(GLenum mode)
      {
         //cout << mode << endl;
         ReBuildAll(); 
         switch( m_sNodeShape ){
   ...
   template <class Cfg, class WEIGHT>
      void CCModel<Cfg, WEIGHT>::DrawRobotNodes(GLenum mode) {
         ...
         float o_c[4]; memcpy(o_c,m_pRobot->GetColor(),4*sizeof(float));
         m_pRobot->BackUp();
         //m_DID_ROBOT = glGenLists(1);
         //glNewList( m_DID_ROBOT, GL_COMPILE );
         if( mode==GL_RENDER ){
         ...
         m_pRobot->Restore();
         m_pRobot->SetColor(o_c[0],o_c[1],o_c[2],o_c[3]);
   ...
   template <class Cfg, class WEIGHT>
      void CCModel<Cfg, WEIGHT>::Draw( GLenum mode ) {
         ...
         switch( m_sNodeShape ){ 
            case Robot: 
               if(m_DID_ROBOT==-1) BuildNodeModels(mode);
               list=m_DID_ROBOT;
               m_shape='r'; 
               break;

            case Box: 
               if(m_DID_Box==-1) BuildNodeModels(mode);
               list=m_DID_Box; 
               m_shape='b';
               break;

            case Point: 
               if(m_DID_PT==-1) BuildNodeModels(mode);
               list=m_DID_PT; 
               m_shape='p';
               break;
         }

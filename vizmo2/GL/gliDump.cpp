...
*/
#ifdef _WIN32
string convert_file="Z:/tool/convert";
void setConvert(const string& name)
{
	convert_file=name;
}
#endif
...
#ifdef _WIN32
inline void replace( string& file )
{
	int len=file.length();
	for(int i=0;i<len;i++)
		if( file[i]=='/' ) file[i]='\\';
...
{
   //filenames
   string ppmFilename=filename;
   ppmFilename=ppmFilename+".ppm";
   ...
   if( imagewrite(ppmFilename.c_str(), new_image)==false ) return false;
   free(new_image.pixels);

   char command[1024];
#ifdef _WIN32
   sprintf(command,"%s %s %s",convert_file.c_str(),ppmFilename.c_str(),saveFilename.c_str());
#else
   sprintf(command,"convert %s %s",ppmFilename.c_str(),saveFilename.c_str());
#endif
   if(system(command)!=0) return false;
#ifdef _WIN32
   replace(ppmFilename);
   sprintf(command,"del %s",ppmFilename.c_str());
#else
   sprintf(command,"rm -f %s",ppmFilename.c_str());
#endif
   if(system(command)!=0) return false;

...
bool CollisionDetection::IsInCollision(int numMB, 
  ...
  rapid = new Rapid();
  //cout<<"CollisionDetection::IsInCollision:: TEST NODE is:: "<<TestNode<<endl;  
  if(TestNode){
    //cout<<"going to copy node Cfg to rapid"<<endl;
...
////////////////////////////////////////////////
  ...
  for(int i = 0; i<numMB; i++){ 
    if(MBI[i].m_active){ 
      ...
      m_robot =  MBmodel[i];
      robIndx = i;
      //cout<<"#### CollisionDetection::IsInCollision \nINDEX OF ROBOT::"<< robIndx <<endl;
      break;
    } 
  }
...
void Rapid::RCopyNodeCfg(double * n_cfg, int dof){
  nodeCfg = new double[dof];
  //cout<<"NodeCfg in RAPID class::"<<endl;
  for(int i=0;i<dof;i++){
    nodeCfg[i] = n_cfg[i];
    //printf("%4f\t", nodeCfg[i]);
  }
  //cout<<"\nnode cfg copied in rapid"<<endl;
}
...
bool Rapid::IsInCollision(CMultiBodyModel * robot, 
  ...
  //    cout<<"-------------------------------------------------------------"<<endl;
  //    printf("ROBOT Poly0 pos(tx, ty, tz):: %2f, %2f, %2f\n", rPoly[0].tx(), rPoly[0].ty(), rPoly[0].tz());
//   printf("ROBOT Poly ori(rx, ry, rz):: %2f, %2f, %2f\n", rPoly[0].rx(), rPoly[0].ry(), rPoly[0].rz());
//   printf("ROBOT MB pos(m_PosPoly):: %2f, %2f, %2f\n",robot->m_PosPoly[0],robot->m_PosPoly[1],robot->m_PosPoly[2]);
//   printf("ROBOT MB pos(tx, ty, tz):: %2f, %2f, %2f\n",robot->tx(),robot->ty(),robot->tz());
//   printf("ROBOT MB rot(rx, ry, rz):: %2f, %2f, %2f\n",robot->rx(),robot->ry(),robot->rz());
//   printf("ROBOT MB rot:: %2f, %2f, %2f\n",robot->m_RotPoly[0],robot->m_RotPoly[1],robot->m_RotPoly[2]);
//   cout<<"-------------------------------------------------------------"<<endl;
  ...
  for(int i=0; i<NumBodiesRob; i++){
    ...
    for(int j=0; j<NumBodiesObs; j++){
      ...
      //cout<< "TESTING LINK " << i << endl;
      if(!test_node){
	//cout<<"****************BY HAND ROBOT******************"<<endl;
	//cout<<"-------------------------------------------------------------"<<endl;
	//printf("ROBOT Poly0 pos(tx, ty, tz):: %2f, %2f, %2f\n", rPoly[0].tx(), rPoly[0].ty(), rPoly[0].tz());
...
// 	    cout<<"-------------------------------------------------------------"<<endl;
// 	    printf("ROBOT Poly %d:: %2f, %2f, %2f\n", i, rPoly[i].tx(), rPoly[i].ty(), rPoly[i].tz());
// 	    cout<<"-------------------------------------------------------------"<<endl;
// 	    cout<<"-------------------------------------------------------------"<<endl;

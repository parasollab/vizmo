...
bool AddObjDial::create(){
  ...
  fn=QFileDialog::getOpenFileName(QString::null,"BYU Files (*.g)",this);
  ...
  if(!fn.isEmpty() ){
    ...
    controls->addWidget(new QLabel(fn.data(), this));
...
void AddObjDial::createMBInfo(){
  ...
  mbiTmp[0].m_pBodyInfo[0].m_strModelDataFileName = fn.data();
...
void AddObjDial::commands(){
  ...
  string s_copy = "cp "+string(fn.data())+ " " +s_path + dirname;
  int value;
  if(!FOUND){
  ...
  m_fileName = "./"+dirname+"/"+getSubstr(string(fn.data()), '/', 1);
